...
template<typename VCLReal> class alignas(VCLReal::vec_bytes) VCLWaveformPEProcessor_NEW:
  public calin::simulation::pe_processor::SimpleListPEProcessor
{
public:
  ...
  VCLWaveformPEProcessor_NEW(unsigned nscope, unsigned npix, unsigned nsample, 
      double time_resolution_ns, unsigned nsample_advance,
      RNG* rng = nullptr, bool auto_clear = true, bool adopt_rng = false):
    calin::simulation::pe_processor::SimpleListPEProcessor(nscope, npix, auto_clear),
    ...
    rng_(rng), adopt_rng_(adopt_rng)
  {
    ...
    if(rng_ == nullptr) {
      rng_ = new RNG(__PRETTY_FUNCTION__, "VCLWaveformPEProcessor_NEW RNG");
      adopt_rng_ = true;
    }
  }

  virtual ~VCLWaveformPEProcessor_NEW() 
  {
    if(adopt_rng_) {
      delete rng_;
    }
