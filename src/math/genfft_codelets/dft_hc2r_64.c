/* Generated by: /Users/sfegan/GitHub/fftw3/genfft/gen_r2cb.native -n 64 -sign 1 -standalone -fma -generic-arith -compact -name dft_codelet_hc2r_64 */

/*
 * This function contains 394 FP additions, 216 FP multiplications,
 * (or, 178 additions, 0 multiplications, 216 fused multiply/add),
 * 109 stack variables, 18 constants, and 128 memory accesses
 */
void dft_codelet_hc2r_64(R * R0, R * R1, const R * Cr, const R * Ci)
{
  DK(KP1_546020906, +1.546020906725473921621813219516939601942082586);
  DK(KP820678790, +0.820678790828660330972281985331011598767386482);
  DK(KP1_990369453, +1.990369453344393772489673906218959843150949737);
  DK(KP098491403, +0.098491403357164253077197521291327432293052451);
  DK(KP1_763842528, +1.763842528696710059425513727320776699016885241);
  DK(KP534511135, +0.534511135950791641089685961295362908582039528);
  DK(KP1_913880671, +1.913880671464417729871595773960539938965698411);
  DK(KP303346683, +0.303346683607342391675883946941299872384187453);
  DK(KP923879532, +0.923879532511286756128183189396788286822416626);
  DK(KP1_662939224, +1.662939224605090474157576755235811513477121624);
  DK(KP668178637, +0.668178637919298919997757686523080761552472251);
  DK(KP1_961570560, +1.961570560806460898252364472268478073947867462);
  DK(KP198912367, +0.198912367379658006911597622644676228597850501);
  DK(KP707106781, +0.707106781186547524400844362104849039284835938);
  DK(KP1_847759065, +1.847759065022573512256366378793576573644833252);
  DK(KP414213562, +0.414213562373095048801688724209698078569671875);
  DK(KP1_414213562, +1.414213562373095048801688724209698078569671875);
  DK(KP2_000000000, +2.000000000000000000000000000000000000000000000);
  {
    
    {
      E T9, T5H, T4p, T5j, T1b, T2T, T3j, T3Z, Tg, T5I, T1m, T2U, T3m,
       T40, T4u;
      E T5k, T1s, T3o, T3r, T1J, To, Tv, T5K, T5L, T5M, T5N, T4A, T5n,
       T1D, T3s;
      E T4F, T5m, T1M, T3p, T1U, T3w, T3H, T2z, TM, T5Q, T5Y, T6f, T4M,
       T5q, T25;
      E T3I, T53, T5t, T2C, T3x, T11, T5V, T4W, T55, T5T, T6g, T2h,
       T2E, T2s, T2F;
      E T3E, T3L, T4R, T54, T3B, T3K;
      {
        E T4, T14, T3, T13, T8, T16, T19, T4o, T1, T2, T5, T4n;
        T4 = Cr[WSCR(16)];
        T14 = Ci[WSCI(16)];
        T1 = Cr[0];
        T2 = Cr[WSCR(32)];
        T3 = ADD(T1, T2);
        T13 = SUB(T1, T2);
        {
          E T6, T7, T17, T18;
          T6 = Cr[WSCR(8)];
          T7 = Cr[WSCR(24)];
          T8 = ADD(T6, T7);
          T16 = SUB(T6, T7);
          T17 = Ci[WSCI(8)];
          T18 = Ci[WSCI(24)];
          T19 = ADD(T17, T18);
          T4o = SUB(T17, T18);
        }
        T5 = FMA(KP2_000000000, T4, T3);
        T9 = FMA(KP2_000000000, T8, T5);
        T5H = FNMS(KP2_000000000, T8, T5);
        T4n = FNMS(KP2_000000000, T4, T3);
        T4p = FNMS(KP2_000000000, T4o, T4n);
        T5j = FMA(KP2_000000000, T4o, T4n);
        {
          E T15, T1a, T3h, T3i;
          T15 = FNMS(KP2_000000000, T14, T13);
          T1a = SUB(T16, T19);
          T1b = FMA(KP1_414213562, T1a, T15);
          T2T = FNMS(KP1_414213562, T1a, T15);
          T3h = FMA(KP2_000000000, T14, T13);
          T3i = ADD(T16, T19);
          T3j = FNMS(KP1_414213562, T3i, T3h);
          T3Z = FMA(KP1_414213562, T3i, T3h);
        }
      }
      {
        E Tc, T1c, T1k, T4r, Tf, T1h, T1f, T4s, T1g, T1l;
        {
          E Ta, Tb, T1i, T1j;
          Ta = Cr[WSCR(4)];
          Tb = Cr[WSCR(28)];
          Tc = ADD(Ta, Tb);
          T1c = SUB(Ta, Tb);
          T1i = Ci[WSCI(4)];
          T1j = Ci[WSCI(28)];
          T1k = ADD(T1i, T1j);
          T4r = SUB(T1i, T1j);
        }
        {
          E Td, Te, T1d, T1e;
          Td = Cr[WSCR(20)];
          Te = Cr[WSCR(12)];
          Tf = ADD(Td, Te);
          T1h = SUB(Td, Te);
          T1d = Ci[WSCI(20)];
          T1e = Ci[WSCI(12)];
          T1f = ADD(T1d, T1e);
          T4s = SUB(T1d, T1e);
        }
        Tg = ADD(Tc, Tf);
        T5I = ADD(T4s, T4r);
        T1g = SUB(T1c, T1f);
        T1l = ADD(T1h, T1k);
        T1m = FNMS(KP414213562, T1l, T1g);
        T2U = FMA(KP414213562, T1g, T1l);
        {
          E T3k, T3l, T4q, T4t;
          T3k = SUB(T1k, T1h);
          T3l = ADD(T1c, T1f);
          T3m = FNMS(KP414213562, T3l, T3k);
          T40 = FMA(KP414213562, T3k, T3l);
          T4q = SUB(Tc, Tf);
          T4t = SUB(T4r, T4s);
          T4u = SUB(T4q, T4t);
          T5k = ADD(T4q, T4t);
        }
      }
      {
        E Tk, T1o, T1I, T4C, Tn, T1F, T1r, T4D, Tr, T1t, T1w, T4y, Tu,
         T1y, T1B;
        E T4x;
        {
          E Ti, Tj, T1G, T1H;
          Ti = Cr[WSCR(2)];
          Tj = Cr[WSCR(30)];
          Tk = ADD(Ti, Tj);
          T1o = SUB(Ti, Tj);
          T1G = Ci[WSCI(2)];
          T1H = Ci[WSCI(30)];
          T1I = ADD(T1G, T1H);
          T4C = SUB(T1G, T1H);
        }
        {
          E Tl, Tm, T1p, T1q;
          Tl = Cr[WSCR(18)];
          Tm = Cr[WSCR(14)];
          Tn = ADD(Tl, Tm);
          T1F = SUB(Tl, Tm);
          T1p = Ci[WSCI(18)];
          T1q = Ci[WSCI(14)];
          T1r = ADD(T1p, T1q);
          T4D = SUB(T1p, T1q);
        }
        {
          E Tp, Tq, T1u, T1v;
          Tp = Cr[WSCR(10)];
          Tq = Cr[WSCR(22)];
          Tr = ADD(Tp, Tq);
          T1t = SUB(Tp, Tq);
          T1u = Ci[WSCI(10)];
          T1v = Ci[WSCI(22)];
          T1w = ADD(T1u, T1v);
          T4y = SUB(T1u, T1v);
        }
        {
          E Ts, Tt, T1z, T1A;
          Ts = Cr[WSCR(6)];
          Tt = Cr[WSCR(26)];
          Tu = ADD(Ts, Tt);
          T1y = SUB(Ts, Tt);
          T1z = Ci[WSCI(6)];
          T1A = Ci[WSCI(26)];
          T1B = ADD(T1z, T1A);
          T4x = SUB(T1A, T1z);
        }
        T1s = SUB(T1o, T1r);
        T3o = ADD(T1o, T1r);
        T3r = SUB(T1I, T1F);
        T1J = ADD(T1F, T1I);
        To = ADD(Tk, Tn);
        Tv = ADD(Tr, Tu);
        T5K = SUB(To, Tv);
        {
          E T4w, T4z, T1x, T1C;
          T5L = ADD(T4D, T4C);
          T5M = ADD(T4y, T4x);
          T5N = SUB(T5L, T5M);
          T4w = SUB(Tk, Tn);
          T4z = SUB(T4x, T4y);
          T4A = ADD(T4w, T4z);
          T5n = SUB(T4w, T4z);
          T1x = SUB(T1t, T1w);
          T1C = SUB(T1y, T1B);
          T1D = ADD(T1x, T1C);
          T3s = SUB(T1C, T1x);
          {
            E T4B, T4E, T1K, T1L;
            T4B = SUB(Tr, Tu);
            T4E = SUB(T4C, T4D);
            T4F = ADD(T4B, T4E);
            T5m = SUB(T4E, T4B);
            T1K = ADD(T1t, T1w);
            T1L = ADD(T1y, T1B);
            T1M = SUB(T1K, T1L);
            T3p = ADD(T1K, T1L);
          }
        }
      }
      {
        E TA, T1Q, T2y, T50, TD, T2v, T1T, T51, TH, T1V, T1Y, T4K, TK,
         T20, T23;
        E T4J;
        {
          E Ty, Tz, T2w, T2x;
          Ty = Cr[WSCR(1)];
          Tz = Cr[WSCR(31)];
          TA = ADD(Ty, Tz);
          T1Q = SUB(Ty, Tz);
          T2w = Ci[WSCI(1)];
          T2x = Ci[WSCI(31)];
          T2y = ADD(T2w, T2x);
          T50 = SUB(T2w, T2x);
        }
        {
          E TB, TC, T1R, T1S;
          TB = Cr[WSCR(17)];
          TC = Cr[WSCR(15)];
          TD = ADD(TB, TC);
          T2v = SUB(TB, TC);
          T1R = Ci[WSCI(17)];
          T1S = Ci[WSCI(15)];
          T1T = ADD(T1R, T1S);
          T51 = SUB(T1R, T1S);
        }
        {
          E TF, TG, T1W, T1X;
          TF = Cr[WSCR(9)];
          TG = Cr[WSCR(23)];
          TH = ADD(TF, TG);
          T1V = SUB(TF, TG);
          T1W = Ci[WSCI(9)];
          T1X = Ci[WSCI(23)];
          T1Y = ADD(T1W, T1X);
          T4K = SUB(T1W, T1X);
        }
        {
          E TI, TJ, T21, T22;
          TI = Cr[WSCR(7)];
          TJ = Cr[WSCR(25)];
          TK = ADD(TI, TJ);
          T20 = SUB(TI, TJ);
          T21 = Ci[WSCI(7)];
          T22 = Ci[WSCI(25)];
          T23 = ADD(T21, T22);
          T4J = SUB(T22, T21);
        }
        {
          E TE, TL, T1Z, T24;
          T1U = SUB(T1Q, T1T);
          T3w = ADD(T1Q, T1T);
          T3H = SUB(T2y, T2v);
          T2z = ADD(T2v, T2y);
          TE = ADD(TA, TD);
          TL = ADD(TH, TK);
          TM = ADD(TE, TL);
          T5Q = SUB(TE, TL);
          {
            E T5W, T5X, T4I, T4L;
            T5W = ADD(T51, T50);
            T5X = ADD(T4K, T4J);
            T5Y = SUB(T5W, T5X);
            T6f = ADD(T5X, T5W);
            T4I = SUB(TA, TD);
            T4L = SUB(T4J, T4K);
            T4M = ADD(T4I, T4L);
            T5q = SUB(T4I, T4L);
          }
          T1Z = SUB(T1V, T1Y);
          T24 = SUB(T20, T23);
          T25 = ADD(T1Z, T24);
          T3I = SUB(T24, T1Z);
          {
            E T4Z, T52, T2A, T2B;
            T4Z = SUB(TH, TK);
            T52 = SUB(T50, T51);
            T53 = ADD(T4Z, T52);
            T5t = SUB(T52, T4Z);
            T2A = ADD(T1V, T1Y);
            T2B = ADD(T20, T23);
            T2C = SUB(T2A, T2B);
            T3x = ADD(T2A, T2B);
          }
        }
      }
      {
        E TP, T27, T2f, T4O, TS, T2c, T2a, T4P, TW, T2i, T2q, T4T, TZ,
         T2n, T2l;
        E T4U;
        {
          E TN, TO, T2d, T2e;
          TN = Cr[WSCR(5)];
          TO = Cr[WSCR(27)];
          TP = ADD(TN, TO);
          T27 = SUB(TN, TO);
          T2d = Ci[WSCI(5)];
          T2e = Ci[WSCI(27)];
          T2f = ADD(T2d, T2e);
          T4O = SUB(T2d, T2e);
        }
        {
          E TQ, TR, T28, T29;
          TQ = Cr[WSCR(21)];
          TR = Cr[WSCR(11)];
          TS = ADD(TQ, TR);
          T2c = SUB(TQ, TR);
          T28 = Ci[WSCI(21)];
          T29 = Ci[WSCI(11)];
          T2a = ADD(T28, T29);
          T4P = SUB(T28, T29);
        }
        {
          E TU, TV, T2o, T2p;
          TU = Cr[WSCR(3)];
          TV = Cr[WSCR(29)];
          TW = ADD(TU, TV);
          T2i = SUB(TU, TV);
          T2o = Ci[WSCI(3)];
          T2p = Ci[WSCI(29)];
          T2q = ADD(T2o, T2p);
          T4T = SUB(T2p, T2o);
        }
        {
          E TX, TY, T2j, T2k;
          TX = Cr[WSCR(13)];
          TY = Cr[WSCR(19)];
          TZ = ADD(TX, TY);
          T2n = SUB(TX, TY);
          T2j = Ci[WSCI(13)];
          T2k = Ci[WSCI(19)];
          T2l = ADD(T2j, T2k);
          T4U = SUB(T2j, T2k);
        }
        {
          E TT, T10, T4S, T4V;
          TT = ADD(TP, TS);
          T10 = ADD(TW, TZ);
          T11 = ADD(TT, T10);
          T5V = SUB(TT, T10);
          T4S = SUB(TW, TZ);
          T4V = SUB(T4T, T4U);
          T4W = ADD(T4S, T4V);
          T55 = SUB(T4V, T4S);
        }
        {
          E T5R, T5S, T2b, T2g;
          T5R = ADD(T4U, T4T);
          T5S = ADD(T4P, T4O);
          T5T = SUB(T5R, T5S);
          T6g = ADD(T5S, T5R);
          T2b = SUB(T27, T2a);
          T2g = ADD(T2c, T2f);
          T2h = FNMS(KP414213562, T2g, T2b);
          T2E = FMA(KP414213562, T2b, T2g);
        }
        {
          E T2m, T2r, T3C, T3D;
          T2m = SUB(T2i, T2l);
          T2r = SUB(T2n, T2q);
          T2s = FMA(KP414213562, T2r, T2m);
          T2F = FNMS(KP414213562, T2m, T2r);
          T3C = ADD(T2n, T2q);
          T3D = ADD(T2i, T2l);
          T3E = FNMS(KP414213562, T3D, T3C);
          T3L = FMA(KP414213562, T3C, T3D);
        }
        {
          E T4N, T4Q, T3z, T3A;
          T4N = SUB(TP, TS);
          T4Q = SUB(T4O, T4P);
          T4R = SUB(T4N, T4Q);
          T54 = ADD(T4N, T4Q);
          T3z = SUB(T2f, T2c);
          T3A = ADD(T27, T2a);
          T3B = FNMS(KP414213562, T3A, T3z);
          T3K = FMA(KP414213562, T3z, T3A);
        }
      }
      {
        E T12, T6m, Tx, T6l, Th, Tw;
        T12 = ADD(TM, T11);
        T6m = ADD(T6g, T6f);
        Th = FMA(KP2_000000000, Tg, T9);
        Tw = ADD(To, Tv);
        Tx = FMA(KP2_000000000, Tw, Th);
        T6l = FNMS(KP2_000000000, Tw, Th);
        R0[WSR(16)] = FNMS(KP2_000000000, T12, Tx);
        R0[WSR(24)] = FMA(KP2_000000000, T6m, T6l);
        R0[0] = FMA(KP2_000000000, T12, Tx);
        R0[WSR(8)] = FNMS(KP2_000000000, T6m, T6l);
      }
      {
        E T65, T69, T68, T6a;
        {
          E T63, T64, T66, T67;
          T63 = FMA(KP2_000000000, T5I, T5H);
          T64 = ADD(T5K, T5N);
          T65 = FNMS(KP1_414213562, T64, T63);
          T69 = FMA(KP1_414213562, T64, T63);
          T66 = SUB(T5Y, T5V);
          T67 = SUB(T5Q, T5T);
          T68 = FNMS(KP414213562, T67, T66);
          T6a = FMA(KP414213562, T66, T67);
        }
        R0[WSR(6)] = FNMS(KP1_847759065, T68, T65);
        R0[WSR(30)] = FMA(KP1_847759065, T6a, T69);
        R0[WSR(22)] = FMA(KP1_847759065, T68, T65);
        R0[WSR(14)] = FNMS(KP1_847759065, T6a, T69);
      }
      {
        E T6d, T6j, T6i, T6k;
        {
          E T6b, T6c, T6e, T6h;
          T6b = FNMS(KP2_000000000, Tg, T9);
          T6c = ADD(T5M, T5L);
          T6d = FNMS(KP2_000000000, T6c, T6b);
          T6j = FMA(KP2_000000000, T6c, T6b);
          T6e = SUB(TM, T11);
          T6h = SUB(T6f, T6g);
          T6i = SUB(T6e, T6h);
          T6k = ADD(T6e, T6h);
        }
        R0[WSR(20)] = FNMS(KP1_414213562, T6i, T6d);
        R0[WSR(28)] = FMA(KP1_414213562, T6k, T6j);
        R0[WSR(4)] = FMA(KP1_414213562, T6i, T6d);
        R0[WSR(12)] = FNMS(KP1_414213562, T6k, T6j);
      }
      {
        E T5P, T61, T60, T62;
        {
          E T5J, T5O, T5U, T5Z;
          T5J = FNMS(KP2_000000000, T5I, T5H);
          T5O = SUB(T5K, T5N);
          T5P = FMA(KP1_414213562, T5O, T5J);
          T61 = FNMS(KP1_414213562, T5O, T5J);
          T5U = ADD(T5Q, T5T);
          T5Z = ADD(T5V, T5Y);
          T60 = FNMS(KP414213562, T5Z, T5U);
          T62 = FMA(KP414213562, T5U, T5Z);
        }
        R0[WSR(18)] = FNMS(KP1_847759065, T60, T5P);
        R0[WSR(26)] = FMA(KP1_847759065, T62, T61);
        R0[WSR(2)] = FMA(KP1_847759065, T60, T5P);
        R0[WSR(10)] = FNMS(KP1_847759065, T62, T61);
      }
      {
        E T4Y, T5f, T57, T5e, T4H, T59, T5d, T5h, T4X, T56;
        T4X = ADD(T4R, T4W);
        T4Y = FMA(KP707106781, T4X, T4M);
        T5f = FNMS(KP707106781, T4X, T4M);
        T56 = ADD(T54, T55);
        T57 = FMA(KP707106781, T56, T53);
        T5e = FNMS(KP707106781, T56, T53);
        {
          E T4v, T4G, T5b, T5c;
          T4v = FMA(KP1_414213562, T4u, T4p);
          T4G = FNMS(KP414213562, T4F, T4A);
          T4H = FMA(KP1_847759065, T4G, T4v);
          T59 = FNMS(KP1_847759065, T4G, T4v);
          T5b = FNMS(KP1_414213562, T4u, T4p);
          T5c = FMA(KP414213562, T4A, T4F);
          T5d = FNMS(KP1_847759065, T5c, T5b);
          T5h = FMA(KP1_847759065, T5c, T5b);
        }
        {
          E T58, T5i, T5a, T5g;
          T58 = FNMS(KP198912367, T57, T4Y);
          R0[WSR(17)] = FNMS(KP1_961570560, T58, T4H);
          R0[WSR(1)] = FMA(KP1_961570560, T58, T4H);
          T5i = FMA(KP668178637, T5e, T5f);
          R0[WSR(13)] = FNMS(KP1_662939224, T5i, T5h);
          R0[WSR(29)] = FMA(KP1_662939224, T5i, T5h);
          T5a = FMA(KP198912367, T4Y, T57);
          R0[WSR(9)] = FNMS(KP1_961570560, T5a, T59);
          R0[WSR(25)] = FMA(KP1_961570560, T5a, T59);
          T5g = FNMS(KP668178637, T5f, T5e);
          R0[WSR(5)] = FNMS(KP1_662939224, T5g, T5d);
          R0[WSR(21)] = FMA(KP1_662939224, T5g, T5d);
        }
      }
      {
        E T5s, T5D, T5v, T5C, T5p, T5x, T5B, T5F, T5r, T5u;
        T5r = SUB(T54, T55);
        T5s = FNMS(KP707106781, T5r, T5q);
        T5D = FMA(KP707106781, T5r, T5q);
        T5u = SUB(T4W, T4R);
        T5v = FNMS(KP707106781, T5u, T5t);
        T5C = FMA(KP707106781, T5u, T5t);
        {
          E T5l, T5o, T5z, T5A;
          T5l = FNMS(KP1_414213562, T5k, T5j);
          T5o = FNMS(KP414213562, T5n, T5m);
          T5p = FNMS(KP1_847759065, T5o, T5l);
          T5x = FMA(KP1_847759065, T5o, T5l);
          T5z = FMA(KP1_414213562, T5k, T5j);
          T5A = FMA(KP414213562, T5m, T5n);
          T5B = FNMS(KP1_847759065, T5A, T5z);
          T5F = FMA(KP1_847759065, T5A, T5z);
        }
        {
          E T5w, T5G, T5y, T5E;
          T5w = FNMS(KP668178637, T5v, T5s);
          R0[WSR(19)] = FNMS(KP1_662939224, T5w, T5p);
          R0[WSR(3)] = FMA(KP1_662939224, T5w, T5p);
          T5G = FMA(KP198912367, T5C, T5D);
          R0[WSR(15)] = FNMS(KP1_961570560, T5G, T5F);
          R0[WSR(31)] = FMA(KP1_961570560, T5G, T5F);
          T5y = FMA(KP668178637, T5s, T5v);
          R0[WSR(11)] = FNMS(KP1_662939224, T5y, T5x);
          R0[WSR(27)] = FMA(KP1_662939224, T5y, T5x);
          T5E = FNMS(KP198912367, T5D, T5C);
          R0[WSR(7)] = FNMS(KP1_961570560, T5E, T5B);
          R0[WSR(23)] = FMA(KP1_961570560, T5E, T5B);
        }
      }
      {
        E T3n, T3R, T3u, T3S, T3G, T3V, T3N, T3U, T3q, T3t;
        T3n = FNMS(KP1_847759065, T3m, T3j);
        T3R = FMA(KP1_847759065, T3m, T3j);
        T3q = FNMS(KP707106781, T3p, T3o);
        T3t = FNMS(KP707106781, T3s, T3r);
        T3u = FNMS(KP668178637, T3t, T3q);
        T3S = FMA(KP668178637, T3q, T3t);
        {
          E T3y, T3F, T3J, T3M;
          T3y = FNMS(KP707106781, T3x, T3w);
          T3F = ADD(T3B, T3E);
          T3G = FNMS(KP923879532, T3F, T3y);
          T3V = FMA(KP923879532, T3F, T3y);
          T3J = FNMS(KP707106781, T3I, T3H);
          T3M = SUB(T3K, T3L);
          T3N = FMA(KP923879532, T3M, T3J);
          T3U = FNMS(KP923879532, T3M, T3J);
        }
        {
          E T3v, T3O, T3X, T3Y;
          T3v = FMA(KP1_662939224, T3u, T3n);
          T3O = FNMS(KP303346683, T3N, T3G);
          R1[WSR(17)] = FNMS(KP1_913880671, T3O, T3v);
          R1[WSR(1)] = FMA(KP1_913880671, T3O, T3v);
          T3X = FMA(KP1_662939224, T3S, T3R);
          T3Y = FMA(KP534511135, T3U, T3V);
          R1[WSR(13)] = FNMS(KP1_763842528, T3Y, T3X);
          R1[WSR(29)] = FMA(KP1_763842528, T3Y, T3X);
        }
        {
          E T3P, T3Q, T3T, T3W;
          T3P = FNMS(KP1_662939224, T3u, T3n);
          T3Q = FMA(KP303346683, T3G, T3N);
          R1[WSR(9)] = FNMS(KP1_913880671, T3Q, T3P);
          R1[WSR(25)] = FMA(KP1_913880671, T3Q, T3P);
          T3T = FNMS(KP1_662939224, T3S, T3R);
          T3W = FNMS(KP534511135, T3V, T3U);
          R1[WSR(5)] = FNMS(KP1_763842528, T3W, T3T);
          R1[WSR(21)] = FMA(KP1_763842528, T3W, T3T);
        }
      }
      {
        E T1n, T2L, T1O, T2M, T2u, T2P, T2H, T2O, T1E, T1N;
        T1n = FMA(KP1_847759065, T1m, T1b);
        T2L = FNMS(KP1_847759065, T1m, T1b);
        T1E = FMA(KP707106781, T1D, T1s);
        T1N = FMA(KP707106781, T1M, T1J);
        T1O = FNMS(KP198912367, T1N, T1E);
        T2M = FMA(KP198912367, T1E, T1N);
        {
          E T26, T2t, T2D, T2G;
          T26 = FMA(KP707106781, T25, T1U);
          T2t = ADD(T2h, T2s);
          T2u = FMA(KP923879532, T2t, T26);
          T2P = FNMS(KP923879532, T2t, T26);
          T2D = FMA(KP707106781, T2C, T2z);
          T2G = ADD(T2E, T2F);
          T2H = FMA(KP923879532, T2G, T2D);
          T2O = FNMS(KP923879532, T2G, T2D);
        }
        {
          E T1P, T2I, T2R, T2S;
          T1P = FMA(KP1_961570560, T1O, T1n);
          T2I = FNMS(KP098491403, T2H, T2u);
          R1[WSR(16)] = FNMS(KP1_990369453, T2I, T1P);
          R1[0] = FMA(KP1_990369453, T2I, T1P);
          T2R = FMA(KP1_961570560, T2M, T2L);
          T2S = FMA(KP820678790, T2O, T2P);
          R1[WSR(12)] = FNMS(KP1_546020906, T2S, T2R);
          R1[WSR(28)] = FMA(KP1_546020906, T2S, T2R);
        }
        {
          E T2J, T2K, T2N, T2Q;
          T2J = FNMS(KP1_961570560, T1O, T1n);
          T2K = FMA(KP098491403, T2u, T2H);
          R1[WSR(8)] = FNMS(KP1_990369453, T2K, T2J);
          R1[WSR(24)] = FMA(KP1_990369453, T2K, T2J);
          T2N = FNMS(KP1_961570560, T2M, T2L);
          T2Q = FNMS(KP820678790, T2P, T2O);
          R1[WSR(4)] = FNMS(KP1_546020906, T2Q, T2N);
          R1[WSR(20)] = FMA(KP1_546020906, T2Q, T2N);
        }
      }
      {
        E T41, T4f, T44, T4g, T48, T4j, T4b, T4i, T42, T43;
        T41 = FNMS(KP1_847759065, T40, T3Z);
        T4f = FMA(KP1_847759065, T40, T3Z);
        T42 = FMA(KP707106781, T3s, T3r);
        T43 = FMA(KP707106781, T3p, T3o);
        T44 = FNMS(KP198912367, T43, T42);
        T4g = FMA(KP198912367, T42, T43);
        {
          E T46, T47, T49, T4a;
          T46 = FMA(KP707106781, T3x, T3w);
          T47 = ADD(T3K, T3L);
          T48 = FNMS(KP923879532, T47, T46);
          T4j = FMA(KP923879532, T47, T46);
          T49 = FMA(KP707106781, T3I, T3H);
          T4a = SUB(T3B, T3E);
          T4b = FNMS(KP923879532, T4a, T49);
          T4i = FMA(KP923879532, T4a, T49);
        }
        {
          E T45, T4c, T4l, T4m;
          T45 = FNMS(KP1_961570560, T44, T41);
          T4c = FNMS(KP820678790, T4b, T48);
          R1[WSR(19)] = FNMS(KP1_546020906, T4c, T45);
          R1[WSR(3)] = FMA(KP1_546020906, T4c, T45);
          T4l = FMA(KP1_961570560, T4g, T4f);
          T4m = FMA(KP098491403, T4i, T4j);
          R1[WSR(15)] = FNMS(KP1_990369453, T4m, T4l);
          R1[WSR(31)] = FMA(KP1_990369453, T4m, T4l);
        }
        {
          E T4d, T4e, T4h, T4k;
          T4d = FMA(KP1_961570560, T44, T41);
          T4e = FMA(KP820678790, T48, T4b);
          R1[WSR(11)] = FNMS(KP1_546020906, T4e, T4d);
          R1[WSR(27)] = FMA(KP1_546020906, T4e, T4d);
          T4h = FNMS(KP1_961570560, T4g, T4f);
          T4k = FNMS(KP098491403, T4j, T4i);
          R1[WSR(7)] = FNMS(KP1_990369453, T4k, T4h);
          R1[WSR(23)] = FMA(KP1_990369453, T4k, T4h);
        }
      }
      {
        E T2V, T39, T2Y, T3a, T32, T3d, T35, T3c, T2W, T2X;
        T2V = FNMS(KP1_847759065, T2U, T2T);
        T39 = FMA(KP1_847759065, T2U, T2T);
        T2W = FNMS(KP707106781, T1M, T1J);
        T2X = FNMS(KP707106781, T1D, T1s);
        T2Y = FNMS(KP668178637, T2X, T2W);
        T3a = FMA(KP668178637, T2W, T2X);
        {
          E T30, T31, T33, T34;
          T30 = FNMS(KP707106781, T25, T1U);
          T31 = SUB(T2E, T2F);
          T32 = FNMS(KP923879532, T31, T30);
          T3d = FMA(KP923879532, T31, T30);
          T33 = FNMS(KP707106781, T2C, T2z);
          T34 = SUB(T2s, T2h);
          T35 = FNMS(KP923879532, T34, T33);
          T3c = FMA(KP923879532, T34, T33);
        }
        {
          E T2Z, T36, T3f, T3g;
          T2Z = FNMS(KP1_662939224, T2Y, T2V);
          T36 = FNMS(KP534511135, T35, T32);
          R1[WSR(18)] = FNMS(KP1_763842528, T36, T2Z);
          R1[WSR(2)] = FMA(KP1_763842528, T36, T2Z);
          T3f = FMA(KP1_662939224, T3a, T39);
          T3g = FMA(KP303346683, T3c, T3d);
          R1[WSR(14)] = FNMS(KP1_913880671, T3g, T3f);
          R1[WSR(30)] = FMA(KP1_913880671, T3g, T3f);
        }
        {
          E T37, T38, T3b, T3e;
          T37 = FMA(KP1_662939224, T2Y, T2V);
          T38 = FMA(KP534511135, T32, T35);
          R1[WSR(10)] = FNMS(KP1_763842528, T38, T37);
          R1[WSR(26)] = FMA(KP1_763842528, T38, T37);
          T3b = FNMS(KP1_662939224, T3a, T39);
          T3e = FNMS(KP303346683, T3d, T3c);
          R1[WSR(6)] = FNMS(KP1_913880671, T3e, T3b);
          R1[WSR(22)] = FMA(KP1_913880671, T3e, T3b);
        }
      }
    }
  }
}
