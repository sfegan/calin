/* Generated by: /Users/sfegan/GitHub/fftw3/genfft/gen_r2cf.native -n 64 -standalone -fma -generic-arith -compact -name dft_codelet_r2hc_64 */

/*
 * This function contains 394 FP additions, 196 FP multiplications,
 * (or, 198 additions, 0 multiplications, 196 fused multiply/add),
 * 106 stack variables, 15 constants, and 128 memory accesses
 */
void dft_codelet_r2hc_64(const R * R0, const R * R1, R * Cr, R * Ci)
{
  DK(KP773010453, +0.773010453362736960810906609758469800971041293);
  DK(KP995184726, +0.995184726672196886244836953109479921575474869);
  DK(KP098491403, +0.098491403357164253077197521291327432293052451);
  DK(KP820678790, +0.820678790828660330972281985331011598767386482);
  DK(KP956940335, +0.956940335732208864935797886980269969482849206);
  DK(KP881921264, +0.881921264348355029712756863660388349508442621);
  DK(KP534511135, +0.534511135950791641089685961295362908582039528);
  DK(KP303346683, +0.303346683607342391675883946941299872384187453);
  DK(KP980785280, +0.980785280403230449126182236134239036973933731);
  DK(KP198912367, +0.198912367379658006911597622644676228597850501);
  DK(KP831469612, +0.831469612302545237078788377617905756738560812);
  DK(KP668178637, +0.668178637919298919997757686523080761552472251);
  DK(KP923879532, +0.923879532511286756128183189396788286822416626);
  DK(KP414213562, +0.414213562373095048801688724209698078569671875);
  DK(KP707106781, +0.707106781186547524400844362104849039284835938);
  {
    
    {
      E T11, T2j, T3D, T5p, T4P, T5P, T7, Te, Tf, T1k, T1H, T4a, T5A,
       T4l, T5D;
      E T2U, T3i, T1R, T2e, T4v, T5H, T4G, T5K, T31, T3l, T3Z, T5t,
       T42, T5s, TZ;
      E T3f, T1b, T2n, T3Q, T5w, T3T, T5v, TK, T3e, T18, T2m, Tm, Tt,
       Tu, T4S;
      E T5q, T14, T2k, T3K, T5Q, T1z, T1I, T4o, T5B, T2X, T3j, T4h,
       T5E, T26, T2f;
      E T4J, T5I, T34, T3m, T4C, T5L;
      {
        E T3, T3z, Td, T3B, T6, T4N, Ta, T3A, T3C, T4O;
        {
          E T1, T2, Tb, Tc;
          T1 = R0[0];
          T2 = R0[WSR(16)];
          T3 = ADD(T1, T2);
          T3z = SUB(T1, T2);
          Tb = R0[WSR(28)];
          Tc = R0[WSR(12)];
          Td = ADD(Tb, Tc);
          T3B = SUB(Tb, Tc);
        }
        {
          E T4, T5, T8, T9;
          T4 = R0[WSR(8)];
          T5 = R0[WSR(24)];
          T6 = ADD(T4, T5);
          T4N = SUB(T4, T5);
          T8 = R0[WSR(4)];
          T9 = R0[WSR(20)];
          Ta = ADD(T8, T9);
          T3A = SUB(T8, T9);
        }
        T11 = SUB(T3, T6);
        T2j = SUB(Td, Ta);
        T3C = ADD(T3A, T3B);
        T3D = FMA(KP707106781, T3C, T3z);
        T5p = FNMS(KP707106781, T3C, T3z);
        T4O = SUB(T3B, T3A);
        T4P = FNMS(KP707106781, T4O, T4N);
        T5P = FMA(KP707106781, T4O, T4N);
        T7 = ADD(T3, T6);
        Te = ADD(Ta, Td);
        Tf = ADD(T7, Te);
      }
      {
        E T1g, T46, T1G, T47, T1j, T4j, T1D, T48;
        {
          E T1e, T1f, T1E, T1F;
          T1e = R1[0];
          T1f = R1[WSR(16)];
          T1g = ADD(T1e, T1f);
          T46 = SUB(T1e, T1f);
          T1E = R1[WSR(4)];
          T1F = R1[WSR(20)];
          T1G = ADD(T1E, T1F);
          T47 = SUB(T1E, T1F);
        }
        {
          E T1h, T1i, T1B, T1C;
          T1h = R1[WSR(8)];
          T1i = R1[WSR(24)];
          T1j = ADD(T1h, T1i);
          T4j = SUB(T1h, T1i);
          T1B = R1[WSR(28)];
          T1C = R1[WSR(12)];
          T1D = ADD(T1B, T1C);
          T48 = SUB(T1B, T1C);
        }
        T1k = SUB(T1g, T1j);
        T1H = SUB(T1D, T1G);
        {
          E T49, T4k, T2S, T2T;
          T49 = ADD(T47, T48);
          T4a = FMA(KP707106781, T49, T46);
          T5A = FNMS(KP707106781, T49, T46);
          T4k = SUB(T47, T48);
          T4l = FMA(KP707106781, T4k, T4j);
          T5D = FNMS(KP707106781, T4k, T4j);
          T2S = ADD(T1g, T1j);
          T2T = ADD(T1G, T1D);
          T2U = ADD(T2S, T2T);
          T3i = SUB(T2S, T2T);
        }
      }
      {
        E T1N, T4r, T2d, T4s, T1Q, T4E, T2a, T4t;
        {
          E T1L, T1M, T2b, T2c;
          T1L = R1[WSR(31)];
          T1M = R1[WSR(15)];
          T1N = ADD(T1L, T1M);
          T4r = SUB(T1L, T1M);
          T2b = R1[WSR(3)];
          T2c = R1[WSR(19)];
          T2d = ADD(T2b, T2c);
          T4s = SUB(T2b, T2c);
        }
        {
          E T1O, T1P, T28, T29;
          T1O = R1[WSR(7)];
          T1P = R1[WSR(23)];
          T1Q = ADD(T1O, T1P);
          T4E = SUB(T1P, T1O);
          T28 = R1[WSR(27)];
          T29 = R1[WSR(11)];
          T2a = ADD(T28, T29);
          T4t = SUB(T28, T29);
        }
        T1R = SUB(T1N, T1Q);
        T2e = SUB(T2a, T2d);
        {
          E T4u, T4F, T2Z, T30;
          T4u = ADD(T4s, T4t);
          T4v = FMA(KP707106781, T4u, T4r);
          T5H = FNMS(KP707106781, T4u, T4r);
          T4F = SUB(T4t, T4s);
          T4G = FMA(KP707106781, T4F, T4E);
          T5K = FNMS(KP707106781, T4F, T4E);
          T2Z = ADD(T1N, T1Q);
          T30 = ADD(T2d, T2a);
          T31 = ADD(T2Z, T30);
          T3l = SUB(T2Z, T30);
        }
      }
      {
        E TN, T3V, TX, T3X, TQ, T40, TU, T3W, T3Y, T41;
        {
          E TL, TM, TV, TW;
          TL = R0[WSR(31)];
          TM = R0[WSR(15)];
          TN = ADD(TL, TM);
          T3V = SUB(TL, TM);
          TV = R0[WSR(27)];
          TW = R0[WSR(11)];
          TX = ADD(TV, TW);
          T3X = SUB(TV, TW);
        }
        {
          E TO, TP, TS, TT;
          TO = R0[WSR(7)];
          TP = R0[WSR(23)];
          TQ = ADD(TO, TP);
          T40 = SUB(TO, TP);
          TS = R0[WSR(3)];
          TT = R0[WSR(19)];
          TU = ADD(TS, TT);
          T3W = SUB(TS, TT);
        }
        T3Y = ADD(T3W, T3X);
        T3Z = FMA(KP707106781, T3Y, T3V);
        T5t = FNMS(KP707106781, T3Y, T3V);
        T41 = SUB(T3W, T3X);
        T42 = FMA(KP707106781, T41, T40);
        T5s = FNMS(KP707106781, T41, T40);
        {
          E TR, TY, T19, T1a;
          TR = ADD(TN, TQ);
          TY = ADD(TU, TX);
          TZ = ADD(TR, TY);
          T3f = SUB(TR, TY);
          T19 = SUB(TN, TQ);
          T1a = SUB(TX, TU);
          T1b = FNMS(KP414213562, T1a, T19);
          T2n = FMA(KP414213562, T19, T1a);
        }
      }
      {
        E Ty, T3M, TI, T3O, TB, T3R, TF, T3N, T3P, T3S;
        {
          E Tw, Tx, TG, TH;
          Tw = R0[WSR(1)];
          Tx = R0[WSR(17)];
          Ty = ADD(Tw, Tx);
          T3M = SUB(Tw, Tx);
          TG = R0[WSR(29)];
          TH = R0[WSR(13)];
          TI = ADD(TG, TH);
          T3O = SUB(TG, TH);
        }
        {
          E Tz, TA, TD, TE;
          Tz = R0[WSR(9)];
          TA = R0[WSR(25)];
          TB = ADD(Tz, TA);
          T3R = SUB(Tz, TA);
          TD = R0[WSR(5)];
          TE = R0[WSR(21)];
          TF = ADD(TD, TE);
          T3N = SUB(TD, TE);
        }
        T3P = ADD(T3N, T3O);
        T3Q = FMA(KP707106781, T3P, T3M);
        T5w = FNMS(KP707106781, T3P, T3M);
        T3S = SUB(T3N, T3O);
        T3T = FMA(KP707106781, T3S, T3R);
        T5v = FNMS(KP707106781, T3S, T3R);
        {
          E TC, TJ, T16, T17;
          TC = ADD(Ty, TB);
          TJ = ADD(TF, TI);
          TK = ADD(TC, TJ);
          T3e = SUB(TC, TJ);
          T16 = SUB(Ty, TB);
          T17 = SUB(TI, TF);
          T18 = FMA(KP414213562, T17, T16);
          T2m = FNMS(KP414213562, T16, T17);
        }
      }
      {
        E Ti, T3E, Ts, T3I, Tl, T3F, Tp, T3H, T4Q, T4R;
        {
          E Tg, Th, Tq, Tr;
          Tg = R0[WSR(2)];
          Th = R0[WSR(18)];
          Ti = ADD(Tg, Th);
          T3E = SUB(Tg, Th);
          Tq = R0[WSR(6)];
          Tr = R0[WSR(22)];
          Ts = ADD(Tq, Tr);
          T3I = SUB(Tq, Tr);
        }
        {
          E Tj, Tk, Tn, To;
          Tj = R0[WSR(10)];
          Tk = R0[WSR(26)];
          Tl = ADD(Tj, Tk);
          T3F = SUB(Tj, Tk);
          Tn = R0[WSR(30)];
          To = R0[WSR(14)];
          Tp = ADD(Tn, To);
          T3H = SUB(Tn, To);
        }
        Tm = ADD(Ti, Tl);
        Tt = ADD(Tp, Ts);
        Tu = ADD(Tm, Tt);
        T4Q = FMA(KP414213562, T3E, T3F);
        T4R = FNMS(KP414213562, T3H, T3I);
        T4S = ADD(T4Q, T4R);
        T5q = SUB(T4Q, T4R);
        {
          E T12, T13, T3G, T3J;
          T12 = SUB(Ti, Tl);
          T13 = SUB(Tp, Ts);
          T14 = ADD(T12, T13);
          T2k = SUB(T13, T12);
          T3G = FNMS(KP414213562, T3F, T3E);
          T3J = FMA(KP414213562, T3I, T3H);
          T3K = ADD(T3G, T3J);
          T5Q = SUB(T3J, T3G);
        }
      }
      {
        E T1n, T4b, T1x, T4f, T1q, T4c, T1u, T4e;
        {
          E T1l, T1m, T1v, T1w;
          T1l = R1[WSR(2)];
          T1m = R1[WSR(18)];
          T1n = ADD(T1l, T1m);
          T4b = SUB(T1l, T1m);
          T1v = R1[WSR(6)];
          T1w = R1[WSR(22)];
          T1x = ADD(T1v, T1w);
          T4f = SUB(T1v, T1w);
        }
        {
          E T1o, T1p, T1s, T1t;
          T1o = R1[WSR(10)];
          T1p = R1[WSR(26)];
          T1q = ADD(T1o, T1p);
          T4c = SUB(T1o, T1p);
          T1s = R1[WSR(30)];
          T1t = R1[WSR(14)];
          T1u = ADD(T1s, T1t);
          T4e = SUB(T1s, T1t);
        }
        {
          E T1r, T1y, T4m, T4n;
          T1r = SUB(T1n, T1q);
          T1y = SUB(T1u, T1x);
          T1z = ADD(T1r, T1y);
          T1I = SUB(T1y, T1r);
          T4m = FMA(KP414213562, T4b, T4c);
          T4n = FNMS(KP414213562, T4e, T4f);
          T4o = ADD(T4m, T4n);
          T5B = SUB(T4m, T4n);
        }
        {
          E T2V, T2W, T4d, T4g;
          T2V = ADD(T1n, T1q);
          T2W = ADD(T1u, T1x);
          T2X = ADD(T2V, T2W);
          T3j = SUB(T2W, T2V);
          T4d = FNMS(KP414213562, T4c, T4b);
          T4g = FMA(KP414213562, T4f, T4e);
          T4h = ADD(T4d, T4g);
          T5E = SUB(T4g, T4d);
        }
      }
      {
        E T1U, T4w, T24, T4A, T1X, T4x, T21, T4z;
        {
          E T1S, T1T, T22, T23;
          T1S = R1[WSR(1)];
          T1T = R1[WSR(17)];
          T1U = ADD(T1S, T1T);
          T4w = SUB(T1S, T1T);
          T22 = R1[WSR(5)];
          T23 = R1[WSR(21)];
          T24 = ADD(T22, T23);
          T4A = SUB(T23, T22);
        }
        {
          E T1V, T1W, T1Z, T20;
          T1V = R1[WSR(9)];
          T1W = R1[WSR(25)];
          T1X = ADD(T1V, T1W);
          T4x = SUB(T1W, T1V);
          T1Z = R1[WSR(29)];
          T20 = R1[WSR(13)];
          T21 = ADD(T1Z, T20);
          T4z = SUB(T1Z, T20);
        }
        {
          E T1Y, T25, T4H, T4I;
          T1Y = SUB(T1U, T1X);
          T25 = SUB(T21, T24);
          T26 = ADD(T1Y, T25);
          T2f = SUB(T25, T1Y);
          T4H = FNMS(KP414213562, T4w, T4x);
          T4I = FMA(KP414213562, T4z, T4A);
          T4J = ADD(T4H, T4I);
          T5I = SUB(T4I, T4H);
        }
        {
          E T32, T33, T4y, T4B;
          T32 = ADD(T1U, T1X);
          T33 = ADD(T21, T24);
          T34 = ADD(T32, T33);
          T3m = SUB(T33, T32);
          T4y = FMA(KP414213562, T4x, T4w);
          T4B = FNMS(KP414213562, T4A, T4z);
          T4C = ADD(T4y, T4B);
          T5L = SUB(T4B, T4y);
        }
      }
      {
        E Tv, T10, T39, T3a, T3b, T3c;
        Tv = ADD(Tf, Tu);
        T10 = ADD(TK, TZ);
        T39 = ADD(Tv, T10);
        T3a = ADD(T2U, T2X);
        T3b = ADD(T31, T34);
        T3c = ADD(T3a, T3b);
        Cr[WSCR(16)] = SUB(Tv, T10);
        Ci[WSCI(16)] = SUB(T3b, T3a);
        Cr[WSCR(32)] = SUB(T39, T3c);
        Cr[0] = ADD(T39, T3c);
      }
      {
        E T2R, T37, T36, T38, T2Y, T35;
        T2R = SUB(Tf, Tu);
        T37 = SUB(TZ, TK);
        T2Y = SUB(T2U, T2X);
        T35 = SUB(T31, T34);
        T36 = ADD(T2Y, T35);
        T38 = SUB(T35, T2Y);
        Cr[WSCR(24)] = FNMS(KP707106781, T36, T2R);
        Ci[WSCI(24)] = NEG(FNMS(KP707106781, T38, T37));
        Cr[WSCR(8)] = FMA(KP707106781, T36, T2R);
        Ci[WSCI(8)] = FMA(KP707106781, T38, T37);
      }
      {
        E T3h, T3x, T3w, T3y, T3o, T3s, T3r, T3t;
        {
          E T3d, T3g, T3u, T3v;
          T3d = SUB(T7, Te);
          T3g = ADD(T3e, T3f);
          T3h = FMA(KP707106781, T3g, T3d);
          T3x = FNMS(KP707106781, T3g, T3d);
          T3u = FNMS(KP414213562, T3i, T3j);
          T3v = FMA(KP414213562, T3l, T3m);
          T3w = ADD(T3u, T3v);
          T3y = SUB(T3v, T3u);
        }
        {
          E T3k, T3n, T3p, T3q;
          T3k = FMA(KP414213562, T3j, T3i);
          T3n = FNMS(KP414213562, T3m, T3l);
          T3o = ADD(T3k, T3n);
          T3s = SUB(T3n, T3k);
          T3p = SUB(Tt, Tm);
          T3q = SUB(T3f, T3e);
          T3r = FNMS(KP707106781, T3q, T3p);
          T3t = FMA(KP707106781, T3q, T3p);
        }
        Cr[WSCR(28)] = FNMS(KP923879532, T3o, T3h);
        Ci[WSCI(28)] = NEG(FNMS(KP923879532, T3w, T3t));
        Cr[WSCR(4)] = FMA(KP923879532, T3o, T3h);
        Ci[WSCI(4)] = FMA(KP923879532, T3w, T3t);
        Ci[WSCI(12)] = NEG(FNMS(KP923879532, T3s, T3r));
        Cr[WSCR(12)] = FMA(KP923879532, T3y, T3x);
        Ci[WSCI(20)] = FMA(KP923879532, T3s, T3r);
        Cr[WSCR(20)] = FNMS(KP923879532, T3y, T3x);
      }
      {
        E T2z, T2P, T2J, T2L, T2C, T2M, T2F, T2N;
        {
          E T2x, T2y, T2H, T2I;
          T2x = FNMS(KP707106781, T14, T11);
          T2y = SUB(T2n, T2m);
          T2z = FMA(KP923879532, T2y, T2x);
          T2P = FNMS(KP923879532, T2y, T2x);
          T2H = FNMS(KP707106781, T2k, T2j);
          T2I = SUB(T1b, T18);
          T2J = FMA(KP923879532, T2I, T2H);
          T2L = FNMS(KP923879532, T2I, T2H);
        }
        {
          E T2A, T2B, T2D, T2E;
          T2A = FNMS(KP707106781, T1z, T1k);
          T2B = FNMS(KP707106781, T1I, T1H);
          T2C = FNMS(KP668178637, T2B, T2A);
          T2M = FMA(KP668178637, T2A, T2B);
          T2D = FNMS(KP707106781, T26, T1R);
          T2E = FNMS(KP707106781, T2f, T2e);
          T2F = FMA(KP668178637, T2E, T2D);
          T2N = FNMS(KP668178637, T2D, T2E);
        }
        {
          E T2G, T2O, T2K, T2Q;
          T2G = ADD(T2C, T2F);
          Cr[WSCR(26)] = FNMS(KP831469612, T2G, T2z);
          Cr[WSCR(6)] = FMA(KP831469612, T2G, T2z);
          T2O = ADD(T2M, T2N);
          Ci[WSCI(6)] = NEG(FMA(KP831469612, T2O, T2L));
          Ci[WSCI(26)] = FNMS(KP831469612, T2O, T2L);
          T2K = SUB(T2F, T2C);
          Ci[WSCI(10)] = FMA(KP831469612, T2K, T2J);
          Ci[WSCI(22)] = NEG(FNMS(KP831469612, T2K, T2J));
          T2Q = SUB(T2M, T2N);
          Cr[WSCR(22)] = FNMS(KP831469612, T2Q, T2P);
          Cr[WSCR(10)] = FMA(KP831469612, T2Q, T2P);
        }
      }
      {
        E T1d, T2v, T2p, T2r, T1K, T2s, T2h, T2t;
        {
          E T15, T1c, T2l, T2o;
          T15 = FMA(KP707106781, T14, T11);
          T1c = ADD(T18, T1b);
          T1d = FMA(KP923879532, T1c, T15);
          T2v = FNMS(KP923879532, T1c, T15);
          T2l = FMA(KP707106781, T2k, T2j);
          T2o = ADD(T2m, T2n);
          T2p = FNMS(KP923879532, T2o, T2l);
          T2r = FMA(KP923879532, T2o, T2l);
        }
        {
          E T1A, T1J, T27, T2g;
          T1A = FMA(KP707106781, T1z, T1k);
          T1J = FMA(KP707106781, T1I, T1H);
          T1K = FMA(KP198912367, T1J, T1A);
          T2s = FNMS(KP198912367, T1A, T1J);
          T27 = FMA(KP707106781, T26, T1R);
          T2g = FMA(KP707106781, T2f, T2e);
          T2h = FNMS(KP198912367, T2g, T27);
          T2t = FMA(KP198912367, T27, T2g);
        }
        {
          E T2i, T2u, T2q, T2w;
          T2i = ADD(T1K, T2h);
          Cr[WSCR(30)] = FNMS(KP980785280, T2i, T1d);
          Cr[WSCR(2)] = FMA(KP980785280, T2i, T1d);
          T2u = ADD(T2s, T2t);
          Ci[WSCI(2)] = FMA(KP980785280, T2u, T2r);
          Ci[WSCI(30)] = NEG(FNMS(KP980785280, T2u, T2r));
          T2q = SUB(T2h, T1K);
          Ci[WSCI(14)] = NEG(FNMS(KP980785280, T2q, T2p));
          Ci[WSCI(18)] = FMA(KP980785280, T2q, T2p);
          T2w = SUB(T2t, T2s);
          Cr[WSCR(18)] = FNMS(KP980785280, T2w, T2v);
          Cr[WSCR(14)] = FMA(KP980785280, T2w, T2v);
        }
      }
      {
        E T5r, T63, T6d, T5R, T5y, T6e, T6b, T6j, T5U, T64, T5G, T5Z,
         T68, T6i, T5N;
        E T5Y;
        {
          E T5u, T5x, T5C, T5F;
          T5r = FNMS(KP923879532, T5q, T5p);
          T63 = FMA(KP923879532, T5q, T5p);
          T6d = FMA(KP923879532, T5Q, T5P);
          T5R = FNMS(KP923879532, T5Q, T5P);
          T5u = FMA(KP668178637, T5t, T5s);
          T5x = FNMS(KP668178637, T5w, T5v);
          T5y = SUB(T5u, T5x);
          T6e = ADD(T5x, T5u);
          {
            E T69, T6a, T5S, T5T;
            T69 = FMA(KP923879532, T5I, T5H);
            T6a = FNMS(KP923879532, T5L, T5K);
            T6b = FMA(KP303346683, T6a, T69);
            T6j = FNMS(KP303346683, T69, T6a);
            T5S = FNMS(KP668178637, T5s, T5t);
            T5T = FMA(KP668178637, T5v, T5w);
            T5U = SUB(T5S, T5T);
            T64 = ADD(T5T, T5S);
          }
          T5C = FNMS(KP923879532, T5B, T5A);
          T5F = FNMS(KP923879532, T5E, T5D);
          T5G = FNMS(KP534511135, T5F, T5C);
          T5Z = FMA(KP534511135, T5C, T5F);
          {
            E T66, T67, T5J, T5M;
            T66 = FMA(KP923879532, T5B, T5A);
            T67 = FMA(KP923879532, T5E, T5D);
            T68 = FMA(KP303346683, T67, T66);
            T6i = FNMS(KP303346683, T66, T67);
            T5J = FNMS(KP923879532, T5I, T5H);
            T5M = FMA(KP923879532, T5L, T5K);
            T5N = FNMS(KP534511135, T5M, T5J);
            T5Y = FMA(KP534511135, T5J, T5M);
          }
        }
        {
          E T5z, T5O, T5X, T60;
          T5z = FMA(KP831469612, T5y, T5r);
          T5O = ADD(T5G, T5N);
          Cr[WSCR(27)] = FNMS(KP881921264, T5O, T5z);
          Cr[WSCR(5)] = FMA(KP881921264, T5O, T5z);
          T5X = FNMS(KP831469612, T5U, T5R);
          T60 = SUB(T5Y, T5Z);
          Ci[WSCI(5)] = NEG(FNMS(KP881921264, T60, T5X));
          Ci[WSCI(27)] = FMA(KP881921264, T60, T5X);
        }
        {
          E T5V, T5W, T61, T62;
          T5V = FMA(KP831469612, T5U, T5R);
          T5W = SUB(T5N, T5G);
          Ci[WSCI(11)] = FMA(KP881921264, T5W, T5V);
          Ci[WSCI(21)] = NEG(FNMS(KP881921264, T5W, T5V));
          T61 = FNMS(KP831469612, T5y, T5r);
          T62 = ADD(T5Z, T5Y);
          Cr[WSCR(21)] = FNMS(KP881921264, T62, T61);
          Cr[WSCR(11)] = FMA(KP881921264, T62, T61);
        }
        {
          E T65, T6c, T6h, T6k;
          T65 = FMA(KP831469612, T64, T63);
          T6c = ADD(T68, T6b);
          Cr[WSCR(29)] = FNMS(KP956940335, T6c, T65);
          Cr[WSCR(3)] = FMA(KP956940335, T6c, T65);
          T6h = FMA(KP831469612, T6e, T6d);
          T6k = SUB(T6i, T6j);
          Ci[WSCI(3)] = FMA(KP956940335, T6k, T6h);
          Ci[WSCI(29)] = NEG(FNMS(KP956940335, T6k, T6h));
        }
        {
          E T6f, T6g, T6l, T6m;
          T6f = FNMS(KP831469612, T6e, T6d);
          T6g = SUB(T6b, T68);
          Ci[WSCI(13)] = NEG(FNMS(KP956940335, T6g, T6f));
          Ci[WSCI(19)] = FMA(KP956940335, T6g, T6f);
          T6l = FNMS(KP831469612, T64, T63);
          T6m = ADD(T6i, T6j);
          Cr[WSCR(19)] = FMA(KP956940335, T6m, T6l);
          Cr[WSCR(13)] = FNMS(KP956940335, T6m, T6l);
        }
      }
      {
        E T3L, T55, T5f, T4T, T44, T5g, T5d, T5l, T4W, T56, T4q, T51,
         T5a, T5k, T4L;
        E T50;
        {
          E T3U, T43, T4i, T4p;
          T3L = FMA(KP923879532, T3K, T3D);
          T55 = FNMS(KP923879532, T3K, T3D);
          T5f = FNMS(KP923879532, T4S, T4P);
          T4T = FMA(KP923879532, T4S, T4P);
          T3U = FNMS(KP198912367, T3T, T3Q);
          T43 = FMA(KP198912367, T42, T3Z);
          T44 = ADD(T3U, T43);
          T5g = SUB(T43, T3U);
          {
            E T5b, T5c, T4U, T4V;
            T5b = FNMS(KP923879532, T4C, T4v);
            T5c = FNMS(KP923879532, T4J, T4G);
            T5d = FMA(KP820678790, T5c, T5b);
            T5l = FNMS(KP820678790, T5b, T5c);
            T4U = FMA(KP198912367, T3Q, T3T);
            T4V = FNMS(KP198912367, T3Z, T42);
            T4W = ADD(T4U, T4V);
            T56 = SUB(T4U, T4V);
          }
          T4i = FMA(KP923879532, T4h, T4a);
          T4p = FMA(KP923879532, T4o, T4l);
          T4q = FNMS(KP098491403, T4p, T4i);
          T51 = FMA(KP098491403, T4i, T4p);
          {
            E T58, T59, T4D, T4K;
            T58 = FNMS(KP923879532, T4h, T4a);
            T59 = FNMS(KP923879532, T4o, T4l);
            T5a = FMA(KP820678790, T59, T58);
            T5k = FNMS(KP820678790, T58, T59);
            T4D = FMA(KP923879532, T4C, T4v);
            T4K = FMA(KP923879532, T4J, T4G);
            T4L = FNMS(KP098491403, T4K, T4D);
            T50 = FMA(KP098491403, T4D, T4K);
          }
        }
        {
          E T45, T4M, T4Z, T52;
          T45 = FMA(KP980785280, T44, T3L);
          T4M = ADD(T4q, T4L);
          Cr[WSCR(31)] = FNMS(KP995184726, T4M, T45);
          Cr[WSCR(1)] = FMA(KP995184726, T4M, T45);
          T4Z = FMA(KP980785280, T4W, T4T);
          T52 = SUB(T50, T51);
          Ci[WSCI(1)] = NEG(FNMS(KP995184726, T52, T4Z));
          Ci[WSCI(31)] = FMA(KP995184726, T52, T4Z);
        }
        {
          E T4X, T4Y, T53, T54;
          T4X = FNMS(KP980785280, T4W, T4T);
          T4Y = SUB(T4L, T4q);
          Ci[WSCI(15)] = FMA(KP995184726, T4Y, T4X);
          Ci[WSCI(17)] = NEG(FNMS(KP995184726, T4Y, T4X));
          T53 = FNMS(KP980785280, T44, T3L);
          T54 = ADD(T51, T50);
          Cr[WSCR(17)] = FNMS(KP995184726, T54, T53);
          Cr[WSCR(15)] = FMA(KP995184726, T54, T53);
        }
        {
          E T57, T5e, T5j, T5m;
          T57 = FMA(KP980785280, T56, T55);
          T5e = ADD(T5a, T5d);
          Cr[WSCR(25)] = FNMS(KP773010453, T5e, T57);
          Cr[WSCR(7)] = FMA(KP773010453, T5e, T57);
          T5j = FMA(KP980785280, T5g, T5f);
          T5m = SUB(T5k, T5l);
          Ci[WSCI(7)] = FMA(KP773010453, T5m, T5j);
          Ci[WSCI(25)] = NEG(FNMS(KP773010453, T5m, T5j));
        }
        {
          E T5h, T5i, T5n, T5o;
          T5h = FNMS(KP980785280, T5g, T5f);
          T5i = SUB(T5d, T5a);
          Ci[WSCI(9)] = NEG(FNMS(KP773010453, T5i, T5h));
          Ci[WSCI(23)] = FMA(KP773010453, T5i, T5h);
          T5n = FNMS(KP980785280, T56, T55);
          T5o = ADD(T5k, T5l);
          Cr[WSCR(23)] = FMA(KP773010453, T5o, T5n);
          Cr[WSCR(9)] = FNMS(KP773010453, T5o, T5n);
        }
      }
    }
  }
}
