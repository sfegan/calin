/* Generated by: /Users/sfegan/GitHub/fftw3/genfft/gen_r2cf.native -n 128 -standalone -fma -generic-arith -compact -name dft_codelet_r2hc_128 */

/*
 * This function contains 956 FP additions, 516 FP multiplications,
 * (or, 440 additions, 0 multiplications, 516 fused multiply/add),
 * 186 stack variables, 31 constants, and 256 memory accesses
 */
void dft_codelet_r2hc_128(const R * R0, const R * R1, R * Cr, R * Ci)
{
  DK(KP803207531, +0.803207531480644909806676512963141923879569427);
  DK(KP989176509, +0.989176509964780973451673738016243063983689533);
  DK(KP741650546, +0.741650546272035369581266691172079863842265220);
  DK(KP148335987, +0.148335987538347428753676511486911367000625355);
  DK(KP740951125, +0.740951125354959091175616897495162729728955309);
  DK(KP998795456, +0.998795456205172392714771604759100694443203615);
  DK(KP906347169, +0.906347169019147157946142717268914412664134293);
  DK(KP049126849, +0.049126849769467254105343321271313617079695752);
  DK(KP970031253, +0.970031253194543992603984207286100251456865962);
  DK(KP857728610, +0.857728610000272069902269984284770137042490799);
  DK(KP250486960, +0.250486960191305461595702160124721208578685568);
  DK(KP599376933, +0.599376933681923766271389869014404232837890546);
  DK(KP941544065, +0.941544065183020778412509402599502357185589796);
  DK(KP903989293, +0.903989293123443331586200297230537048710132025);
  DK(KP357805721, +0.357805721314524104672487743774474392487532769);
  DK(KP472964775, +0.472964775891319928124438237972992463904131113);
  DK(KP773010453, +0.773010453362736960810906609758469800971041293);
  DK(KP995184726, +0.995184726672196886244836953109479921575474869);
  DK(KP098491403, +0.098491403357164253077197521291327432293052451);
  DK(KP820678790, +0.820678790828660330972281985331011598767386482);
  DK(KP956940335, +0.956940335732208864935797886980269969482849206);
  DK(KP881921264, +0.881921264348355029712756863660388349508442621);
  DK(KP534511135, +0.534511135950791641089685961295362908582039528);
  DK(KP303346683, +0.303346683607342391675883946941299872384187453);
  DK(KP831469612, +0.831469612302545237078788377617905756738560812);
  DK(KP980785280, +0.980785280403230449126182236134239036973933731);
  DK(KP923879532, +0.923879532511286756128183189396788286822416626);
  DK(KP668178637, +0.668178637919298919997757686523080761552472251);
  DK(KP198912367, +0.198912367379658006911597622644676228597850501);
  DK(KP414213562, +0.414213562373095048801688724209698078569671875);
  DK(KP707106781, +0.707106781186547524400844362104849039284835938);
  {
    
    {
      E TcD, TdR, T27, T7r, T5P, T8v, Tf, Ta5, Tu, Tbn, TcG, TdS, T2e,
       T8w, T5S;
      E T7s, TK, Ta6, TcK, TdU, T2o, T5U, T7w, T8y, TZ, Ta7, TcN, TdV,
       T2x, T5V;
      E T7z, T8z, T1g, Taa, TcU, Tex, TcX, Tew, T1v, Tab, T2M, T6A,
       T7E, T9b, T7H;
      E T9a, T2T, T6z, T4X, T6L, Tdz, TeL, TdK, TeO, T5G, T6O, T8d,
       T9p, TaV, Tc3;
      E Tbi, Tc4, T8o, T9s, T3I, T6E, Tde, TeE, Tdp, TeH, T4r, T6H,
       T7U, T9i, Tao;
      E TbW, TaL, TbX, T85, T9l, T1L, Tad, Td3, TeA, Td6, Tez, T20,
       Tae, T37, T6x;
      E T7L, T9e, T7O, T9d, T3e, T6w, TbZ, Tc0, T3Z, T4s, Tds, TeF,
       T4g, T4t, T80;
      E T87, Tdl, TeI, T7X, T86, TaD, TaM, Tc6, Tc7, T5e, T5H, TdN,
       TeM, T5v, T5I;
      E T8j, T8q, TdG, TeP, T8g, T8p, Tba, Tbj;
      {
        E T3, T23, Td, T25, T6, T5N, Ta, T24;
        {
          E T1, T2, Tb, Tc;
          T1 = R0[0];
          T2 = R0[WSR(32)];
          T3 = ADD(T1, T2);
          T23 = SUB(T1, T2);
          Tb = R0[WSR(56)];
          Tc = R0[WSR(24)];
          Td = ADD(Tb, Tc);
          T25 = SUB(Tb, Tc);
        }
        {
          E T4, T5, T8, T9;
          T4 = R0[WSR(16)];
          T5 = R0[WSR(48)];
          T6 = ADD(T4, T5);
          T5N = SUB(T4, T5);
          T8 = R0[WSR(8)];
          T9 = R0[WSR(40)];
          Ta = ADD(T8, T9);
          T24 = SUB(T8, T9);
        }
        TcD = SUB(T3, T6);
        TdR = SUB(Td, Ta);
        {
          E T26, T5O, T7, Te;
          T26 = ADD(T24, T25);
          T27 = FMA(KP707106781, T26, T23);
          T7r = FNMS(KP707106781, T26, T23);
          T5O = SUB(T25, T24);
          T5P = FNMS(KP707106781, T5O, T5N);
          T8v = FMA(KP707106781, T5O, T5N);
          T7 = ADD(T3, T6);
          Te = ADD(Ta, Td);
          Tf = ADD(T7, Te);
          Ta5 = SUB(T7, Te);
        }
      }
      {
        E Ti, T28, Ts, T2c, Tl, T29, Tp, T2b;
        {
          E Tg, Th, Tq, Tr;
          Tg = R0[WSR(4)];
          Th = R0[WSR(36)];
          Ti = ADD(Tg, Th);
          T28 = SUB(Tg, Th);
          Tq = R0[WSR(12)];
          Tr = R0[WSR(44)];
          Ts = ADD(Tq, Tr);
          T2c = SUB(Tq, Tr);
        }
        {
          E Tj, Tk, Tn, To;
          Tj = R0[WSR(20)];
          Tk = R0[WSR(52)];
          Tl = ADD(Tj, Tk);
          T29 = SUB(Tj, Tk);
          Tn = R0[WSR(60)];
          To = R0[WSR(28)];
          Tp = ADD(Tn, To);
          T2b = SUB(Tn, To);
        }
        {
          E Tm, Tt, TcE, TcF;
          Tm = ADD(Ti, Tl);
          Tt = ADD(Tp, Ts);
          Tu = ADD(Tm, Tt);
          Tbn = SUB(Tt, Tm);
          TcE = SUB(Ti, Tl);
          TcF = SUB(Tp, Ts);
          TcG = ADD(TcE, TcF);
          TdS = SUB(TcF, TcE);
        }
        {
          E T2a, T2d, T5Q, T5R;
          T2a = FNMS(KP414213562, T29, T28);
          T2d = FMA(KP414213562, T2c, T2b);
          T2e = ADD(T2a, T2d);
          T8w = SUB(T2d, T2a);
          T5Q = FMA(KP414213562, T28, T29);
          T5R = FNMS(KP414213562, T2b, T2c);
          T5S = ADD(T5Q, T5R);
          T7s = SUB(T5Q, T5R);
        }
      }
      {
        E Ty, T2g, TB, T2l, TF, T2m, TI, T2j;
        {
          E Tw, Tx, Tz, TA;
          Tw = R0[WSR(2)];
          Tx = R0[WSR(34)];
          Ty = ADD(Tw, Tx);
          T2g = SUB(Tw, Tx);
          Tz = R0[WSR(18)];
          TA = R0[WSR(50)];
          TB = ADD(Tz, TA);
          T2l = SUB(Tz, TA);
          {
            E TD, TE, T2h, TG, TH, T2i;
            TD = R0[WSR(10)];
            TE = R0[WSR(42)];
            T2h = SUB(TD, TE);
            TG = R0[WSR(58)];
            TH = R0[WSR(26)];
            T2i = SUB(TG, TH);
            TF = ADD(TD, TE);
            T2m = SUB(T2h, T2i);
            TI = ADD(TG, TH);
            T2j = ADD(T2h, T2i);
          }
        }
        {
          E TC, TJ, TcI, TcJ;
          TC = ADD(Ty, TB);
          TJ = ADD(TF, TI);
          TK = ADD(TC, TJ);
          Ta6 = SUB(TC, TJ);
          TcI = SUB(Ty, TB);
          TcJ = SUB(TI, TF);
          TcK = FMA(KP414213562, TcJ, TcI);
          TdU = FNMS(KP414213562, TcI, TcJ);
        }
        {
          E T2k, T2n, T7u, T7v;
          T2k = FMA(KP707106781, T2j, T2g);
          T2n = FMA(KP707106781, T2m, T2l);
          T2o = FNMS(KP198912367, T2n, T2k);
          T5U = FMA(KP198912367, T2k, T2n);
          T7u = FNMS(KP707106781, T2j, T2g);
          T7v = FNMS(KP707106781, T2m, T2l);
          T7w = FMA(KP668178637, T7v, T7u);
          T8y = FNMS(KP668178637, T7u, T7v);
        }
      }
      {
        E TN, T2p, TQ, T2u, TU, T2v, TX, T2s;
        {
          E TL, TM, TO, TP;
          TL = R0[WSR(62)];
          TM = R0[WSR(30)];
          TN = ADD(TL, TM);
          T2p = SUB(TL, TM);
          TO = R0[WSR(14)];
          TP = R0[WSR(46)];
          TQ = ADD(TO, TP);
          T2u = SUB(TO, TP);
          {
            E TS, TT, T2q, TV, TW, T2r;
            TS = R0[WSR(6)];
            TT = R0[WSR(38)];
            T2q = SUB(TS, TT);
            TV = R0[WSR(54)];
            TW = R0[WSR(22)];
            T2r = SUB(TV, TW);
            TU = ADD(TS, TT);
            T2v = SUB(T2q, T2r);
            TX = ADD(TV, TW);
            T2s = ADD(T2q, T2r);
          }
        }
        {
          E TR, TY, TcL, TcM;
          TR = ADD(TN, TQ);
          TY = ADD(TU, TX);
          TZ = ADD(TR, TY);
          Ta7 = SUB(TR, TY);
          TcL = SUB(TN, TQ);
          TcM = SUB(TX, TU);
          TcN = FNMS(KP414213562, TcM, TcL);
          TdV = FMA(KP414213562, TcL, TcM);
        }
        {
          E T2t, T2w, T7x, T7y;
          T2t = FMA(KP707106781, T2s, T2p);
          T2w = FMA(KP707106781, T2v, T2u);
          T2x = FMA(KP198912367, T2w, T2t);
          T5V = FNMS(KP198912367, T2t, T2w);
          T7x = FNMS(KP707106781, T2s, T2p);
          T7y = FNMS(KP707106781, T2v, T2u);
          T7z = FNMS(KP668178637, T7y, T7x);
          T8z = FMA(KP668178637, T7x, T7y);
        }
      }
      {
        E T14, T2A, T17, T2N, T1b, T2O, T1e, T2D, T1j, T1m, T2H, TcR,
         T2Q, T1q, T1t;
        E T2K, TcS, T2R;
        {
          E T12, T13, T15, T16;
          T12 = R0[WSR(1)];
          T13 = R0[WSR(33)];
          T14 = ADD(T12, T13);
          T2A = SUB(T12, T13);
          T15 = R0[WSR(17)];
          T16 = R0[WSR(49)];
          T17 = ADD(T15, T16);
          T2N = SUB(T15, T16);
        }
        {
          E T19, T1a, T2B, T1c, T1d, T2C;
          T19 = R0[WSR(9)];
          T1a = R0[WSR(41)];
          T2B = SUB(T19, T1a);
          T1c = R0[WSR(57)];
          T1d = R0[WSR(25)];
          T2C = SUB(T1c, T1d);
          T1b = ADD(T19, T1a);
          T2O = SUB(T2B, T2C);
          T1e = ADD(T1c, T1d);
          T2D = ADD(T2B, T2C);
        }
        {
          E T2F, T2G, T2I, T2J;
          {
            E T1h, T1i, T1k, T1l;
            T1h = R0[WSR(5)];
            T1i = R0[WSR(37)];
            T1j = ADD(T1h, T1i);
            T2F = SUB(T1h, T1i);
            T1k = R0[WSR(21)];
            T1l = R0[WSR(53)];
            T1m = ADD(T1k, T1l);
            T2G = SUB(T1k, T1l);
          }
          T2H = FNMS(KP414213562, T2G, T2F);
          TcR = SUB(T1j, T1m);
          T2Q = FMA(KP414213562, T2F, T2G);
          {
            E T1o, T1p, T1r, T1s;
            T1o = R0[WSR(61)];
            T1p = R0[WSR(29)];
            T1q = ADD(T1o, T1p);
            T2I = SUB(T1o, T1p);
            T1r = R0[WSR(13)];
            T1s = R0[WSR(45)];
            T1t = ADD(T1r, T1s);
            T2J = SUB(T1r, T1s);
          }
          T2K = FMA(KP414213562, T2J, T2I);
          TcS = SUB(T1q, T1t);
          T2R = FNMS(KP414213562, T2I, T2J);
        }
        {
          E T18, T1f, TcQ, TcT;
          T18 = ADD(T14, T17);
          T1f = ADD(T1b, T1e);
          T1g = ADD(T18, T1f);
          Taa = SUB(T18, T1f);
          TcQ = SUB(T14, T17);
          TcT = ADD(TcR, TcS);
          TcU = FMA(KP707106781, TcT, TcQ);
          Tex = FNMS(KP707106781, TcT, TcQ);
        }
        {
          E TcV, TcW, T1n, T1u;
          TcV = SUB(T1e, T1b);
          TcW = SUB(TcS, TcR);
          TcX = FMA(KP707106781, TcW, TcV);
          Tew = FNMS(KP707106781, TcW, TcV);
          T1n = ADD(T1j, T1m);
          T1u = ADD(T1q, T1t);
          T1v = ADD(T1n, T1u);
          Tab = SUB(T1u, T1n);
        }
        {
          E T2E, T2L, T7C, T7D;
          T2E = FMA(KP707106781, T2D, T2A);
          T2L = ADD(T2H, T2K);
          T2M = FMA(KP923879532, T2L, T2E);
          T6A = FNMS(KP923879532, T2L, T2E);
          T7C = FNMS(KP707106781, T2D, T2A);
          T7D = SUB(T2Q, T2R);
          T7E = FMA(KP923879532, T7D, T7C);
          T9b = FNMS(KP923879532, T7D, T7C);
        }
        {
          E T7F, T7G, T2P, T2S;
          T7F = FNMS(KP707106781, T2O, T2N);
          T7G = SUB(T2K, T2H);
          T7H = FMA(KP923879532, T7G, T7F);
          T9a = FNMS(KP923879532, T7G, T7F);
          T2P = FMA(KP707106781, T2O, T2N);
          T2S = ADD(T2Q, T2R);
          T2T = FMA(KP923879532, T2S, T2P);
          T6z = FNMS(KP923879532, T2S, T2P);
        }
      }
      {
        E T4z, TaP, T5A, TaQ, T4G, TaT, T5B, TaS, Tbf, Tbg, T4O, Tdw,
         T5D, Tbc, Tbd;
        E T4V, Tdx, T5E;
        {
          E T4x, T4y, T5y, T5z;
          T4x = R1[WSR(63)];
          T4y = R1[WSR(31)];
          T4z = SUB(T4x, T4y);
          TaP = ADD(T4x, T4y);
          T5y = R1[WSR(47)];
          T5z = R1[WSR(15)];
          T5A = SUB(T5y, T5z);
          TaQ = ADD(T5z, T5y);
        }
        {
          E T4A, T4B, T4C, T4D, T4E, T4F;
          T4A = R1[WSR(7)];
          T4B = R1[WSR(39)];
          T4C = SUB(T4A, T4B);
          T4D = R1[WSR(55)];
          T4E = R1[WSR(23)];
          T4F = SUB(T4D, T4E);
          T4G = ADD(T4C, T4F);
          TaT = ADD(T4D, T4E);
          T5B = SUB(T4F, T4C);
          TaS = ADD(T4A, T4B);
        }
        {
          E T4K, T4N, T4R, T4U;
          {
            E T4I, T4J, T4L, T4M;
            T4I = R1[WSR(3)];
            T4J = R1[WSR(35)];
            T4K = SUB(T4I, T4J);
            Tbf = ADD(T4I, T4J);
            T4L = R1[WSR(51)];
            T4M = R1[WSR(19)];
            T4N = SUB(T4L, T4M);
            Tbg = ADD(T4M, T4L);
          }
          T4O = FMA(KP414213562, T4N, T4K);
          Tdw = SUB(Tbf, Tbg);
          T5D = FNMS(KP414213562, T4K, T4N);
          {
            E T4P, T4Q, T4S, T4T;
            T4P = R1[WSR(59)];
            T4Q = R1[WSR(27)];
            T4R = SUB(T4P, T4Q);
            Tbc = ADD(T4P, T4Q);
            T4S = R1[WSR(43)];
            T4T = R1[WSR(11)];
            T4U = SUB(T4S, T4T);
            Tbd = ADD(T4T, T4S);
          }
          T4V = FNMS(KP414213562, T4U, T4R);
          Tdx = SUB(Tbc, Tbd);
          T5E = FMA(KP414213562, T4R, T4U);
        }
        {
          E T4H, T4W, Tdv, Tdy;
          T4H = FMA(KP707106781, T4G, T4z);
          T4W = ADD(T4O, T4V);
          T4X = FMA(KP923879532, T4W, T4H);
          T6L = FNMS(KP923879532, T4W, T4H);
          Tdv = SUB(TaP, TaQ);
          Tdy = ADD(Tdw, Tdx);
          Tdz = FMA(KP707106781, Tdy, Tdv);
          TeL = FNMS(KP707106781, Tdy, Tdv);
        }
        {
          E TdI, TdJ, T5C, T5F;
          TdI = SUB(TaT, TaS);
          TdJ = SUB(Tdx, Tdw);
          TdK = FMA(KP707106781, TdJ, TdI);
          TeO = FNMS(KP707106781, TdJ, TdI);
          T5C = FMA(KP707106781, T5B, T5A);
          T5F = ADD(T5D, T5E);
          T5G = FMA(KP923879532, T5F, T5C);
          T6O = FNMS(KP923879532, T5F, T5C);
        }
        {
          E T8b, T8c, TaR, TaU;
          T8b = FNMS(KP707106781, T4G, T4z);
          T8c = SUB(T5E, T5D);
          T8d = FMA(KP923879532, T8c, T8b);
          T9p = FNMS(KP923879532, T8c, T8b);
          TaR = ADD(TaP, TaQ);
          TaU = ADD(TaS, TaT);
          TaV = SUB(TaR, TaU);
          Tc3 = ADD(TaR, TaU);
        }
        {
          E Tbe, Tbh, T8m, T8n;
          Tbe = ADD(Tbc, Tbd);
          Tbh = ADD(Tbf, Tbg);
          Tbi = SUB(Tbe, Tbh);
          Tc4 = ADD(Tbh, Tbe);
          T8m = FNMS(KP707106781, T5B, T5A);
          T8n = SUB(T4V, T4O);
          T8o = FNMS(KP923879532, T8n, T8m);
          T9s = FMA(KP923879532, T8n, T8m);
        }
      }
      {
        E T3k, Tai, T4l, Taj, T3r, Tam, T4m, Tal, TaI, TaJ, T3z, Tdb,
         T4o, TaF, TaG;
        E T3G, Tdc, T4p;
        {
          E T3i, T3j, T4j, T4k;
          T3i = R1[0];
          T3j = R1[WSR(32)];
          T3k = SUB(T3i, T3j);
          Tai = ADD(T3i, T3j);
          T4j = R1[WSR(16)];
          T4k = R1[WSR(48)];
          T4l = SUB(T4j, T4k);
          Taj = ADD(T4j, T4k);
        }
        {
          E T3l, T3m, T3n, T3o, T3p, T3q;
          T3l = R1[WSR(8)];
          T3m = R1[WSR(40)];
          T3n = SUB(T3l, T3m);
          T3o = R1[WSR(56)];
          T3p = R1[WSR(24)];
          T3q = SUB(T3o, T3p);
          T3r = ADD(T3n, T3q);
          Tam = ADD(T3o, T3p);
          T4m = SUB(T3n, T3q);
          Tal = ADD(T3l, T3m);
        }
        {
          E T3v, T3y, T3C, T3F;
          {
            E T3t, T3u, T3w, T3x;
            T3t = R1[WSR(4)];
            T3u = R1[WSR(36)];
            T3v = SUB(T3t, T3u);
            TaI = ADD(T3t, T3u);
            T3w = R1[WSR(20)];
            T3x = R1[WSR(52)];
            T3y = SUB(T3w, T3x);
            TaJ = ADD(T3w, T3x);
          }
          T3z = FNMS(KP414213562, T3y, T3v);
          Tdb = SUB(TaI, TaJ);
          T4o = FMA(KP414213562, T3v, T3y);
          {
            E T3A, T3B, T3D, T3E;
            T3A = R1[WSR(60)];
            T3B = R1[WSR(28)];
            T3C = SUB(T3A, T3B);
            TaF = ADD(T3A, T3B);
            T3D = R1[WSR(12)];
            T3E = R1[WSR(44)];
            T3F = SUB(T3D, T3E);
            TaG = ADD(T3D, T3E);
          }
          T3G = FMA(KP414213562, T3F, T3C);
          Tdc = SUB(TaF, TaG);
          T4p = FNMS(KP414213562, T3C, T3F);
        }
        {
          E T3s, T3H, Tda, Tdd;
          T3s = FMA(KP707106781, T3r, T3k);
          T3H = ADD(T3z, T3G);
          T3I = FMA(KP923879532, T3H, T3s);
          T6E = FNMS(KP923879532, T3H, T3s);
          Tda = SUB(Tai, Taj);
          Tdd = ADD(Tdb, Tdc);
          Tde = FMA(KP707106781, Tdd, Tda);
          TeE = FNMS(KP707106781, Tdd, Tda);
        }
        {
          E Tdn, Tdo, T4n, T4q;
          Tdn = SUB(Tam, Tal);
          Tdo = SUB(Tdc, Tdb);
          Tdp = FMA(KP707106781, Tdo, Tdn);
          TeH = FNMS(KP707106781, Tdo, Tdn);
          T4n = FMA(KP707106781, T4m, T4l);
          T4q = ADD(T4o, T4p);
          T4r = FMA(KP923879532, T4q, T4n);
          T6H = FNMS(KP923879532, T4q, T4n);
        }
        {
          E T7S, T7T, Tak, Tan;
          T7S = FNMS(KP707106781, T3r, T3k);
          T7T = SUB(T4o, T4p);
          T7U = FMA(KP923879532, T7T, T7S);
          T9i = FNMS(KP923879532, T7T, T7S);
          Tak = ADD(Tai, Taj);
          Tan = ADD(Tal, Tam);
          Tao = SUB(Tak, Tan);
          TbW = ADD(Tak, Tan);
        }
        {
          E TaH, TaK, T83, T84;
          TaH = ADD(TaF, TaG);
          TaK = ADD(TaI, TaJ);
          TaL = SUB(TaH, TaK);
          TbX = ADD(TaK, TaH);
          T83 = FNMS(KP707106781, T4m, T4l);
          T84 = SUB(T3G, T3z);
          T85 = FMA(KP923879532, T84, T83);
          T9l = FNMS(KP923879532, T84, T83);
        }
      }
      {
        E T1z, T2V, T1C, T38, T1G, T39, T1J, T2Y, T1O, T1R, T32, Td0,
         T3b, T1V, T1Y;
        E T35, Td1, T3c;
        {
          E T1x, T1y, T1A, T1B;
          T1x = R0[WSR(63)];
          T1y = R0[WSR(31)];
          T1z = ADD(T1x, T1y);
          T2V = SUB(T1x, T1y);
          T1A = R0[WSR(15)];
          T1B = R0[WSR(47)];
          T1C = ADD(T1A, T1B);
          T38 = SUB(T1A, T1B);
        }
        {
          E T1E, T1F, T2W, T1H, T1I, T2X;
          T1E = R0[WSR(7)];
          T1F = R0[WSR(39)];
          T2W = SUB(T1E, T1F);
          T1H = R0[WSR(55)];
          T1I = R0[WSR(23)];
          T2X = SUB(T1H, T1I);
          T1G = ADD(T1E, T1F);
          T39 = SUB(T2W, T2X);
          T1J = ADD(T1H, T1I);
          T2Y = ADD(T2W, T2X);
        }
        {
          E T30, T31, T33, T34;
          {
            E T1M, T1N, T1P, T1Q;
            T1M = R0[WSR(3)];
            T1N = R0[WSR(35)];
            T1O = ADD(T1M, T1N);
            T30 = SUB(T1M, T1N);
            T1P = R0[WSR(19)];
            T1Q = R0[WSR(51)];
            T1R = ADD(T1P, T1Q);
            T31 = SUB(T1P, T1Q);
          }
          T32 = FNMS(KP414213562, T31, T30);
          Td0 = SUB(T1O, T1R);
          T3b = FMA(KP414213562, T30, T31);
          {
            E T1T, T1U, T1W, T1X;
            T1T = R0[WSR(59)];
            T1U = R0[WSR(27)];
            T1V = ADD(T1T, T1U);
            T33 = SUB(T1T, T1U);
            T1W = R0[WSR(11)];
            T1X = R0[WSR(43)];
            T1Y = ADD(T1W, T1X);
            T34 = SUB(T1W, T1X);
          }
          T35 = FMA(KP414213562, T34, T33);
          Td1 = SUB(T1V, T1Y);
          T3c = FNMS(KP414213562, T33, T34);
        }
        {
          E T1D, T1K, TcZ, Td2;
          T1D = ADD(T1z, T1C);
          T1K = ADD(T1G, T1J);
          T1L = ADD(T1D, T1K);
          Tad = SUB(T1D, T1K);
          TcZ = SUB(T1z, T1C);
          Td2 = ADD(Td0, Td1);
          Td3 = FMA(KP707106781, Td2, TcZ);
          TeA = FNMS(KP707106781, Td2, TcZ);
        }
        {
          E Td4, Td5, T1S, T1Z;
          Td4 = SUB(T1J, T1G);
          Td5 = SUB(Td1, Td0);
          Td6 = FMA(KP707106781, Td5, Td4);
          Tez = FNMS(KP707106781, Td5, Td4);
          T1S = ADD(T1O, T1R);
          T1Z = ADD(T1V, T1Y);
          T20 = ADD(T1S, T1Z);
          Tae = SUB(T1Z, T1S);
        }
        {
          E T2Z, T36, T7J, T7K;
          T2Z = FMA(KP707106781, T2Y, T2V);
          T36 = ADD(T32, T35);
          T37 = FMA(KP923879532, T36, T2Z);
          T6x = FNMS(KP923879532, T36, T2Z);
          T7J = FNMS(KP707106781, T2Y, T2V);
          T7K = SUB(T3b, T3c);
          T7L = FMA(KP923879532, T7K, T7J);
          T9e = FNMS(KP923879532, T7K, T7J);
        }
        {
          E T7M, T7N, T3a, T3d;
          T7M = FNMS(KP707106781, T39, T38);
          T7N = SUB(T35, T32);
          T7O = FMA(KP923879532, T7N, T7M);
          T9d = FNMS(KP923879532, T7N, T7M);
          T3a = FMA(KP707106781, T39, T38);
          T3d = ADD(T3b, T3c);
          T3e = FMA(KP923879532, T3d, T3a);
          T6w = FNMS(KP923879532, T3d, T3a);
        }
      }
      {
        E T3L, Tdf, T3W, Tar, T42, Tdi, T4d, Tay, T3S, Tdg, T3X, Tau,
         T49, Tdj, T4e;
        E TaB, Tdh, Tdk;
        {
          E T3J, T3K, Tap, T3U, T3V, Taq;
          T3J = R1[WSR(2)];
          T3K = R1[WSR(34)];
          Tap = ADD(T3J, T3K);
          T3U = R1[WSR(18)];
          T3V = R1[WSR(50)];
          Taq = ADD(T3U, T3V);
          T3L = SUB(T3J, T3K);
          Tdf = SUB(Tap, Taq);
          T3W = SUB(T3U, T3V);
          Tar = ADD(Tap, Taq);
        }
        {
          E T40, T41, Taw, T4b, T4c, Tax;
          T40 = R1[WSR(62)];
          T41 = R1[WSR(30)];
          Taw = ADD(T40, T41);
          T4b = R1[WSR(14)];
          T4c = R1[WSR(46)];
          Tax = ADD(T4b, T4c);
          T42 = SUB(T40, T41);
          Tdi = SUB(Taw, Tax);
          T4d = SUB(T4b, T4c);
          Tay = ADD(Taw, Tax);
        }
        {
          E T3O, Tas, T3R, Tat;
          {
            E T3M, T3N, T3P, T3Q;
            T3M = R1[WSR(10)];
            T3N = R1[WSR(42)];
            T3O = SUB(T3M, T3N);
            Tas = ADD(T3M, T3N);
            T3P = R1[WSR(58)];
            T3Q = R1[WSR(26)];
            T3R = SUB(T3P, T3Q);
            Tat = ADD(T3P, T3Q);
          }
          T3S = ADD(T3O, T3R);
          Tdg = SUB(Tat, Tas);
          T3X = SUB(T3O, T3R);
          Tau = ADD(Tas, Tat);
        }
        {
          E T45, Taz, T48, TaA;
          {
            E T43, T44, T46, T47;
            T43 = R1[WSR(6)];
            T44 = R1[WSR(38)];
            T45 = SUB(T43, T44);
            Taz = ADD(T43, T44);
            T46 = R1[WSR(54)];
            T47 = R1[WSR(22)];
            T48 = SUB(T46, T47);
            TaA = ADD(T46, T47);
          }
          T49 = ADD(T45, T48);
          Tdj = SUB(TaA, Taz);
          T4e = SUB(T45, T48);
          TaB = ADD(Taz, TaA);
        }
        TbZ = ADD(Tar, Tau);
        Tc0 = ADD(Tay, TaB);
        {
          E T3T, T3Y, Tdq, Tdr;
          T3T = FMA(KP707106781, T3S, T3L);
          T3Y = FMA(KP707106781, T3X, T3W);
          T3Z = FNMS(KP198912367, T3Y, T3T);
          T4s = FMA(KP198912367, T3T, T3Y);
          Tdq = FNMS(KP414213562, Tdf, Tdg);
          Tdr = FMA(KP414213562, Tdi, Tdj);
          Tds = ADD(Tdq, Tdr);
          TeF = SUB(Tdr, Tdq);
        }
        {
          E T4a, T4f, T7Y, T7Z;
          T4a = FMA(KP707106781, T49, T42);
          T4f = FMA(KP707106781, T4e, T4d);
          T4g = FMA(KP198912367, T4f, T4a);
          T4t = FNMS(KP198912367, T4a, T4f);
          T7Y = FNMS(KP707106781, T49, T42);
          T7Z = FNMS(KP707106781, T4e, T4d);
          T80 = FNMS(KP668178637, T7Z, T7Y);
          T87 = FMA(KP668178637, T7Y, T7Z);
        }
        Tdh = FMA(KP414213562, Tdg, Tdf);
        Tdk = FNMS(KP414213562, Tdj, Tdi);
        Tdl = ADD(Tdh, Tdk);
        TeI = SUB(Tdh, Tdk);
        {
          E T7V, T7W, Tav, TaC;
          T7V = FNMS(KP707106781, T3S, T3L);
          T7W = FNMS(KP707106781, T3X, T3W);
          T7X = FMA(KP668178637, T7W, T7V);
          T86 = FNMS(KP668178637, T7V, T7W);
          Tav = SUB(Tar, Tau);
          TaC = SUB(Tay, TaB);
          TaD = ADD(Tav, TaC);
          TaM = SUB(TaC, Tav);
        }
      }
      {
        E T50, TdA, T5b, TaY, T5h, TdD, T5s, Tb5, T57, TdB, T5c, Tb1,
         T5o, TdE, T5t;
        E Tb8, TdC, TdF;
        {
          E T4Y, T4Z, TaW, T59, T5a, TaX;
          T4Y = R1[WSR(1)];
          T4Z = R1[WSR(33)];
          TaW = ADD(T4Y, T4Z);
          T59 = R1[WSR(49)];
          T5a = R1[WSR(17)];
          TaX = ADD(T5a, T59);
          T50 = SUB(T4Y, T4Z);
          TdA = SUB(TaW, TaX);
          T5b = SUB(T59, T5a);
          TaY = ADD(TaW, TaX);
        }
        {
          E T5f, T5g, Tb3, T5q, T5r, Tb4;
          T5f = R1[WSR(61)];
          T5g = R1[WSR(29)];
          Tb3 = ADD(T5f, T5g);
          T5q = R1[WSR(45)];
          T5r = R1[WSR(13)];
          Tb4 = ADD(T5r, T5q);
          T5h = SUB(T5f, T5g);
          TdD = SUB(Tb3, Tb4);
          T5s = SUB(T5q, T5r);
          Tb5 = ADD(Tb3, Tb4);
        }
        {
          E T53, TaZ, T56, Tb0;
          {
            E T51, T52, T54, T55;
            T51 = R1[WSR(9)];
            T52 = R1[WSR(41)];
            T53 = SUB(T51, T52);
            TaZ = ADD(T51, T52);
            T54 = R1[WSR(57)];
            T55 = R1[WSR(25)];
            T56 = SUB(T54, T55);
            Tb0 = ADD(T54, T55);
          }
          T57 = ADD(T53, T56);
          TdB = SUB(Tb0, TaZ);
          T5c = SUB(T56, T53);
          Tb1 = ADD(TaZ, Tb0);
        }
        {
          E T5k, Tb6, T5n, Tb7;
          {
            E T5i, T5j, T5l, T5m;
            T5i = R1[WSR(5)];
            T5j = R1[WSR(37)];
            T5k = SUB(T5i, T5j);
            Tb6 = ADD(T5i, T5j);
            T5l = R1[WSR(53)];
            T5m = R1[WSR(21)];
            T5n = SUB(T5l, T5m);
            Tb7 = ADD(T5l, T5m);
          }
          T5o = ADD(T5k, T5n);
          TdE = SUB(Tb7, Tb6);
          T5t = SUB(T5n, T5k);
          Tb8 = ADD(Tb6, Tb7);
        }
        Tc6 = ADD(TaY, Tb1);
        Tc7 = ADD(Tb5, Tb8);
        {
          E T58, T5d, TdL, TdM;
          T58 = FMA(KP707106781, T57, T50);
          T5d = FMA(KP707106781, T5c, T5b);
          T5e = FMA(KP198912367, T5d, T58);
          T5H = FNMS(KP198912367, T58, T5d);
          TdL = FNMS(KP414213562, TdA, TdB);
          TdM = FMA(KP414213562, TdD, TdE);
          TdN = ADD(TdL, TdM);
          TeM = SUB(TdM, TdL);
        }
        {
          E T5p, T5u, T8h, T8i;
          T5p = FMA(KP707106781, T5o, T5h);
          T5u = FMA(KP707106781, T5t, T5s);
          T5v = FNMS(KP198912367, T5u, T5p);
          T5I = FMA(KP198912367, T5p, T5u);
          T8h = FNMS(KP707106781, T5o, T5h);
          T8i = FNMS(KP707106781, T5t, T5s);
          T8j = FMA(KP668178637, T8i, T8h);
          T8q = FNMS(KP668178637, T8h, T8i);
        }
        TdC = FMA(KP414213562, TdB, TdA);
        TdF = FNMS(KP414213562, TdE, TdD);
        TdG = ADD(TdC, TdF);
        TeP = SUB(TdF, TdC);
        {
          E T8e, T8f, Tb2, Tb9;
          T8e = FNMS(KP707106781, T57, T50);
          T8f = FNMS(KP707106781, T5c, T5b);
          T8g = FNMS(KP668178637, T8f, T8e);
          T8p = FMA(KP668178637, T8e, T8f);
          Tb2 = SUB(TaY, Tb1);
          Tb9 = SUB(Tb5, Tb8);
          Tba = ADD(Tb2, Tb9);
          Tbj = SUB(Tb9, Tb2);
        }
      }
      {
        E T11, TbV, Tc9, Tcf, T22, Tcb, Tc2, Tce;
        {
          E Tv, T10, Tc5, Tc8;
          Tv = ADD(Tf, Tu);
          T10 = ADD(TK, TZ);
          T11 = ADD(Tv, T10);
          TbV = SUB(Tv, T10);
          Tc5 = ADD(Tc3, Tc4);
          Tc8 = ADD(Tc6, Tc7);
          Tc9 = SUB(Tc5, Tc8);
          Tcf = ADD(Tc5, Tc8);
        }
        {
          E T1w, T21, TbY, Tc1;
          T1w = ADD(T1g, T1v);
          T21 = ADD(T1L, T20);
          T22 = ADD(T1w, T21);
          Tcb = SUB(T21, T1w);
          TbY = ADD(TbW, TbX);
          Tc1 = ADD(TbZ, Tc0);
          Tc2 = SUB(TbY, Tc1);
          Tce = ADD(TbY, Tc1);
        }
        Cr[WSCR(32)] = SUB(T11, T22);
        Ci[WSCI(32)] = SUB(Tcf, Tce);
        {
          E Tca, Tcc, Tcd, Tcg;
          Tca = ADD(Tc2, Tc9);
          Cr[WSCR(48)] = FNMS(KP707106781, Tca, TbV);
          Cr[WSCR(16)] = FMA(KP707106781, Tca, TbV);
          Tcc = SUB(Tc9, Tc2);
          Ci[WSCI(16)] = FMA(KP707106781, Tcc, Tcb);
          Ci[WSCI(48)] = NEG(FNMS(KP707106781, Tcc, Tcb));
          Tcd = ADD(T11, T22);
          Tcg = ADD(Tce, Tcf);
          Cr[WSCR(64)] = SUB(Tcd, Tcg);
          Cr[0] = ADD(Tcd, Tcg);
        }
      }
      {
        E Tch, Tct, Tck, Tcu, Tco, Tcy, Tcr, Tcz, Tci, Tcj;
        Tch = SUB(Tf, Tu);
        Tct = SUB(TZ, TK);
        Tci = SUB(T1g, T1v);
        Tcj = SUB(T1L, T20);
        Tck = ADD(Tci, Tcj);
        Tcu = SUB(Tcj, Tci);
        {
          E Tcm, Tcn, Tcp, Tcq;
          Tcm = SUB(TbW, TbX);
          Tcn = SUB(Tc0, TbZ);
          Tco = FMA(KP414213562, Tcn, Tcm);
          Tcy = FNMS(KP414213562, Tcm, Tcn);
          Tcp = SUB(Tc3, Tc4);
          Tcq = SUB(Tc7, Tc6);
          Tcr = FNMS(KP414213562, Tcq, Tcp);
          Tcz = FMA(KP414213562, Tcp, Tcq);
        }
        {
          E Tcl, Tcs, Tcx, TcA;
          Tcl = FMA(KP707106781, Tck, Tch);
          Tcs = ADD(Tco, Tcr);
          Cr[WSCR(56)] = FNMS(KP923879532, Tcs, Tcl);
          Cr[WSCR(8)] = FMA(KP923879532, Tcs, Tcl);
          Tcx = FMA(KP707106781, Tcu, Tct);
          TcA = ADD(Tcy, Tcz);
          Ci[WSCI(8)] = FMA(KP923879532, TcA, Tcx);
          Ci[WSCI(56)] = NEG(FNMS(KP923879532, TcA, Tcx));
        }
        {
          E Tcv, Tcw, TcB, TcC;
          Tcv = FNMS(KP707106781, Tcu, Tct);
          Tcw = SUB(Tcr, Tco);
          Ci[WSCI(24)] = NEG(FNMS(KP923879532, Tcw, Tcv));
          Ci[WSCI(40)] = FMA(KP923879532, Tcw, Tcv);
          TcB = FNMS(KP707106781, Tck, Tch);
          TcC = SUB(Tcz, Tcy);
          Cr[WSCR(40)] = FNMS(KP923879532, TcC, TcB);
          Cr[WSCR(24)] = FMA(KP923879532, TcC, TcB);
        }
      }
      {
        E Ta9, TbB, Tbp, TbL, Tag, TbM, TbJ, TbR, TaO, Tbw, Tbs, TbC,
         TbG, TbQ, Tbl;
        E Tbx, Ta8, Tbo;
        Ta8 = ADD(Ta6, Ta7);
        Ta9 = FMA(KP707106781, Ta8, Ta5);
        TbB = FNMS(KP707106781, Ta8, Ta5);
        Tbo = SUB(Ta7, Ta6);
        Tbp = FMA(KP707106781, Tbo, Tbn);
        TbL = FNMS(KP707106781, Tbo, Tbn);
        {
          E Tac, Taf, TbH, TbI;
          Tac = FMA(KP414213562, Tab, Taa);
          Taf = FNMS(KP414213562, Tae, Tad);
          Tag = ADD(Tac, Taf);
          TbM = SUB(Taf, Tac);
          TbH = FNMS(KP707106781, Tba, TaV);
          TbI = FNMS(KP707106781, Tbj, Tbi);
          TbJ = FMA(KP668178637, TbI, TbH);
          TbR = FNMS(KP668178637, TbH, TbI);
        }
        {
          E TaE, TaN, Tbq, Tbr;
          TaE = FMA(KP707106781, TaD, Tao);
          TaN = FMA(KP707106781, TaM, TaL);
          TaO = FMA(KP198912367, TaN, TaE);
          Tbw = FNMS(KP198912367, TaE, TaN);
          Tbq = FNMS(KP414213562, Taa, Tab);
          Tbr = FMA(KP414213562, Tad, Tae);
          Tbs = ADD(Tbq, Tbr);
          TbC = SUB(Tbr, Tbq);
        }
        {
          E TbE, TbF, Tbb, Tbk;
          TbE = FNMS(KP707106781, TaD, Tao);
          TbF = FNMS(KP707106781, TaM, TaL);
          TbG = FNMS(KP668178637, TbF, TbE);
          TbQ = FMA(KP668178637, TbE, TbF);
          Tbb = FMA(KP707106781, Tba, TaV);
          Tbk = FMA(KP707106781, Tbj, Tbi);
          Tbl = FNMS(KP198912367, Tbk, Tbb);
          Tbx = FMA(KP198912367, Tbb, Tbk);
        }
        {
          E Tah, Tbm, Tbv, Tby;
          Tah = FMA(KP923879532, Tag, Ta9);
          Tbm = ADD(TaO, Tbl);
          Cr[WSCR(60)] = FNMS(KP980785280, Tbm, Tah);
          Cr[WSCR(4)] = FMA(KP980785280, Tbm, Tah);
          Tbv = FMA(KP923879532, Tbs, Tbp);
          Tby = ADD(Tbw, Tbx);
          Ci[WSCI(4)] = FMA(KP980785280, Tby, Tbv);
          Ci[WSCI(60)] = NEG(FNMS(KP980785280, Tby, Tbv));
        }
        {
          E Tbt, Tbu, Tbz, TbA;
          Tbt = FNMS(KP923879532, Tbs, Tbp);
          Tbu = SUB(Tbl, TaO);
          Ci[WSCI(28)] = NEG(FNMS(KP980785280, Tbu, Tbt));
          Ci[WSCI(36)] = FMA(KP980785280, Tbu, Tbt);
          Tbz = FNMS(KP923879532, Tag, Ta9);
          TbA = SUB(Tbx, Tbw);
          Cr[WSCR(36)] = FNMS(KP980785280, TbA, Tbz);
          Cr[WSCR(28)] = FMA(KP980785280, TbA, Tbz);
        }
        {
          E TbD, TbK, TbP, TbS;
          TbD = FMA(KP923879532, TbC, TbB);
          TbK = ADD(TbG, TbJ);
          Cr[WSCR(52)] = FNMS(KP831469612, TbK, TbD);
          Cr[WSCR(12)] = FMA(KP831469612, TbK, TbD);
          TbP = FNMS(KP923879532, TbM, TbL);
          TbS = ADD(TbQ, TbR);
          Ci[WSCI(12)] = NEG(FMA(KP831469612, TbS, TbP));
          Ci[WSCI(52)] = FNMS(KP831469612, TbS, TbP);
        }
        {
          E TbN, TbO, TbT, TbU;
          TbN = FMA(KP923879532, TbM, TbL);
          TbO = SUB(TbJ, TbG);
          Ci[WSCI(20)] = FMA(KP831469612, TbO, TbN);
          Ci[WSCI(44)] = NEG(FNMS(KP831469612, TbO, TbN));
          TbT = FNMS(KP923879532, TbC, TbB);
          TbU = SUB(TbQ, TbR);
          Cr[WSCR(44)] = FNMS(KP831469612, TbU, TbT);
          Cr[WSCR(20)] = FMA(KP831469612, TbU, TbT);
        }
      }
      {
        E Tev, Tf7, Tfc, Tfm, Tff, Tfn, TeC, Tfi, TeK, Tf2, TeV, Tfh,
         TeY, Tf8, TeR;
        E Tf3;
        {
          E Tet, Teu, Tfa, Tfb;
          Tet = FNMS(KP707106781, TcG, TcD);
          Teu = SUB(TdV, TdU);
          Tev = FNMS(KP923879532, Teu, Tet);
          Tf7 = FMA(KP923879532, Teu, Tet);
          Tfa = FMA(KP923879532, TeF, TeE);
          Tfb = FMA(KP923879532, TeI, TeH);
          Tfc = FNMS(KP303346683, Tfb, Tfa);
          Tfm = FMA(KP303346683, Tfa, Tfb);
        }
        {
          E Tfd, Tfe, Tey, TeB;
          Tfd = FMA(KP923879532, TeM, TeL);
          Tfe = FNMS(KP923879532, TeP, TeO);
          Tff = FMA(KP303346683, Tfe, Tfd);
          Tfn = FNMS(KP303346683, Tfd, Tfe);
          Tey = FMA(KP668178637, Tex, Tew);
          TeB = FNMS(KP668178637, TeA, Tez);
          TeC = SUB(Tey, TeB);
          Tfi = ADD(Tey, TeB);
        }
        {
          E TeG, TeJ, TeT, TeU;
          TeG = FNMS(KP923879532, TeF, TeE);
          TeJ = FNMS(KP923879532, TeI, TeH);
          TeK = FMA(KP534511135, TeJ, TeG);
          Tf2 = FNMS(KP534511135, TeG, TeJ);
          TeT = FNMS(KP707106781, TdS, TdR);
          TeU = SUB(TcN, TcK);
          TeV = FMA(KP923879532, TeU, TeT);
          Tfh = FNMS(KP923879532, TeU, TeT);
        }
        {
          E TeW, TeX, TeN, TeQ;
          TeW = FMA(KP668178637, Tez, TeA);
          TeX = FNMS(KP668178637, Tew, Tex);
          TeY = SUB(TeW, TeX);
          Tf8 = ADD(TeX, TeW);
          TeN = FNMS(KP923879532, TeM, TeL);
          TeQ = FMA(KP923879532, TeP, TeO);
          TeR = FNMS(KP534511135, TeQ, TeN);
          Tf3 = FMA(KP534511135, TeN, TeQ);
        }
        {
          E TeD, TeS, Tf1, Tf4;
          TeD = FMA(KP831469612, TeC, Tev);
          TeS = ADD(TeK, TeR);
          Cr[WSCR(54)] = FNMS(KP881921264, TeS, TeD);
          Cr[WSCR(10)] = FMA(KP881921264, TeS, TeD);
          Tf1 = FMA(KP831469612, TeY, TeV);
          Tf4 = ADD(Tf2, Tf3);
          Ci[WSCI(10)] = FMA(KP881921264, Tf4, Tf1);
          Ci[WSCI(54)] = NEG(FNMS(KP881921264, Tf4, Tf1));
        }
        {
          E TeZ, Tf0, Tf5, Tf6;
          TeZ = FNMS(KP831469612, TeY, TeV);
          Tf0 = SUB(TeR, TeK);
          Ci[WSCI(22)] = NEG(FNMS(KP881921264, Tf0, TeZ));
          Ci[WSCI(42)] = FMA(KP881921264, Tf0, TeZ);
          Tf5 = FNMS(KP831469612, TeC, Tev);
          Tf6 = SUB(Tf3, Tf2);
          Cr[WSCR(42)] = FNMS(KP881921264, Tf6, Tf5);
          Cr[WSCR(22)] = FMA(KP881921264, Tf6, Tf5);
        }
        {
          E Tf9, Tfg, Tfl, Tfo;
          Tf9 = FMA(KP831469612, Tf8, Tf7);
          Tfg = ADD(Tfc, Tff);
          Cr[WSCR(58)] = FNMS(KP956940335, Tfg, Tf9);
          Cr[WSCR(6)] = FMA(KP956940335, Tfg, Tf9);
          Tfl = FMA(KP831469612, Tfi, Tfh);
          Tfo = ADD(Tfm, Tfn);
          Ci[WSCI(6)] = NEG(FMA(KP956940335, Tfo, Tfl));
          Ci[WSCI(58)] = FNMS(KP956940335, Tfo, Tfl);
        }
        {
          E Tfj, Tfk, Tfp, Tfq;
          Tfj = FNMS(KP831469612, Tfi, Tfh);
          Tfk = SUB(Tff, Tfc);
          Ci[WSCI(26)] = FMA(KP956940335, Tfk, Tfj);
          Ci[WSCI(38)] = NEG(FNMS(KP956940335, Tfk, Tfj));
          Tfp = FNMS(KP831469612, Tf8, Tf7);
          Tfq = SUB(Tfm, Tfn);
          Cr[WSCR(38)] = FNMS(KP956940335, Tfq, Tfp);
          Cr[WSCR(26)] = FMA(KP956940335, Tfq, Tfp);
        }
      }
      {
        E TcP, Te9, Tee, Teo, Teh, Tep, Td8, Tek, Tdu, Te4, TdX, Tej,
         Te0, Tea, TdP;
        E Te5;
        {
          E TcH, TcO, Tec, Ted;
          TcH = FMA(KP707106781, TcG, TcD);
          TcO = ADD(TcK, TcN);
          TcP = FMA(KP923879532, TcO, TcH);
          Te9 = FNMS(KP923879532, TcO, TcH);
          Tec = FNMS(KP923879532, Tdl, Tde);
          Ted = FNMS(KP923879532, Tds, Tdp);
          Tee = FNMS(KP820678790, Ted, Tec);
          Teo = FMA(KP820678790, Tec, Ted);
        }
        {
          E Tef, Teg, TcY, Td7;
          Tef = FNMS(KP923879532, TdG, Tdz);
          Teg = FNMS(KP923879532, TdN, TdK);
          Teh = FMA(KP820678790, Teg, Tef);
          Tep = FNMS(KP820678790, Tef, Teg);
          TcY = FMA(KP198912367, TcX, TcU);
          Td7 = FNMS(KP198912367, Td6, Td3);
          Td8 = ADD(TcY, Td7);
          Tek = SUB(Td7, TcY);
        }
        {
          E Tdm, Tdt, TdT, TdW;
          Tdm = FMA(KP923879532, Tdl, Tde);
          Tdt = FMA(KP923879532, Tds, Tdp);
          Tdu = FMA(KP098491403, Tdt, Tdm);
          Te4 = FNMS(KP098491403, Tdm, Tdt);
          TdT = FMA(KP707106781, TdS, TdR);
          TdW = ADD(TdU, TdV);
          TdX = FMA(KP923879532, TdW, TdT);
          Tej = FNMS(KP923879532, TdW, TdT);
        }
        {
          E TdY, TdZ, TdH, TdO;
          TdY = FNMS(KP198912367, TcU, TcX);
          TdZ = FMA(KP198912367, Td3, Td6);
          Te0 = ADD(TdY, TdZ);
          Tea = SUB(TdZ, TdY);
          TdH = FMA(KP923879532, TdG, Tdz);
          TdO = FMA(KP923879532, TdN, TdK);
          TdP = FNMS(KP098491403, TdO, TdH);
          Te5 = FMA(KP098491403, TdH, TdO);
        }
        {
          E Td9, TdQ, Te3, Te6;
          Td9 = FMA(KP980785280, Td8, TcP);
          TdQ = ADD(Tdu, TdP);
          Cr[WSCR(62)] = FNMS(KP995184726, TdQ, Td9);
          Cr[WSCR(2)] = FMA(KP995184726, TdQ, Td9);
          Te3 = FMA(KP980785280, Te0, TdX);
          Te6 = ADD(Te4, Te5);
          Ci[WSCI(2)] = FMA(KP995184726, Te6, Te3);
          Ci[WSCI(62)] = NEG(FNMS(KP995184726, Te6, Te3));
        }
        {
          E Te1, Te2, Te7, Te8;
          Te1 = FNMS(KP980785280, Te0, TdX);
          Te2 = SUB(TdP, Tdu);
          Ci[WSCI(30)] = NEG(FNMS(KP995184726, Te2, Te1));
          Ci[WSCI(34)] = FMA(KP995184726, Te2, Te1);
          Te7 = FNMS(KP980785280, Td8, TcP);
          Te8 = SUB(Te5, Te4);
          Cr[WSCR(34)] = FNMS(KP995184726, Te8, Te7);
          Cr[WSCR(30)] = FMA(KP995184726, Te8, Te7);
        }
        {
          E Teb, Tei, Ten, Teq;
          Teb = FMA(KP980785280, Tea, Te9);
          Tei = ADD(Tee, Teh);
          Cr[WSCR(50)] = FNMS(KP773010453, Tei, Teb);
          Cr[WSCR(14)] = FMA(KP773010453, Tei, Teb);
          Ten = FNMS(KP980785280, Tek, Tej);
          Teq = ADD(Teo, Tep);
          Ci[WSCI(14)] = NEG(FMA(KP773010453, Teq, Ten));
          Ci[WSCI(50)] = FNMS(KP773010453, Teq, Ten);
        }
        {
          E Tel, Tem, Ter, Tes;
          Tel = FMA(KP980785280, Tek, Tej);
          Tem = SUB(Teh, Tee);
          Ci[WSCI(18)] = FMA(KP773010453, Tem, Tel);
          Ci[WSCI(46)] = NEG(FNMS(KP773010453, Tem, Tel));
          Ter = FNMS(KP980785280, Tea, Te9);
          Tes = SUB(Teo, Tep);
          Cr[WSCR(46)] = FNMS(KP773010453, Tes, Ter);
          Cr[WSCR(18)] = FMA(KP773010453, Tes, Ter);
        }
      }
      {
        E T6v, T77, T6C, T7i, T6Y, T78, T6V, T7h, T6R, T7n, T72, T7f,
         T6K, T7m, T73;
        E T7c;
        {
          E T6t, T6u, T6T, T6U;
          T6t = FNMS(KP923879532, T2e, T27);
          T6u = SUB(T5U, T5V);
          T6v = FNMS(KP980785280, T6u, T6t);
          T77 = FMA(KP980785280, T6u, T6t);
          {
            E T6y, T6B, T6W, T6X;
            T6y = FMA(KP820678790, T6x, T6w);
            T6B = FNMS(KP820678790, T6A, T6z);
            T6C = SUB(T6y, T6B);
            T7i = ADD(T6B, T6y);
            T6W = FNMS(KP820678790, T6w, T6x);
            T6X = FMA(KP820678790, T6z, T6A);
            T6Y = SUB(T6W, T6X);
            T78 = ADD(T6X, T6W);
          }
          T6T = FNMS(KP923879532, T5S, T5P);
          T6U = SUB(T2x, T2o);
          T6V = FNMS(KP980785280, T6U, T6T);
          T7h = FMA(KP980785280, T6U, T6T);
          {
            E T6N, T7d, T6Q, T7e, T6M, T6P;
            T6M = SUB(T5I, T5H);
            T6N = FNMS(KP980785280, T6M, T6L);
            T7d = FMA(KP980785280, T6M, T6L);
            T6P = SUB(T5v, T5e);
            T6Q = FMA(KP980785280, T6P, T6O);
            T7e = FNMS(KP980785280, T6P, T6O);
            T6R = FNMS(KP472964775, T6Q, T6N);
            T7n = FNMS(KP357805721, T7d, T7e);
            T72 = FMA(KP472964775, T6N, T6Q);
            T7f = FMA(KP357805721, T7e, T7d);
          }
          {
            E T6G, T7a, T6J, T7b, T6F, T6I;
            T6F = SUB(T4s, T4t);
            T6G = FNMS(KP980785280, T6F, T6E);
            T7a = FMA(KP980785280, T6F, T6E);
            T6I = SUB(T4g, T3Z);
            T6J = FNMS(KP980785280, T6I, T6H);
            T7b = FMA(KP980785280, T6I, T6H);
            T6K = FNMS(KP472964775, T6J, T6G);
            T7m = FNMS(KP357805721, T7a, T7b);
            T73 = FMA(KP472964775, T6G, T6J);
            T7c = FMA(KP357805721, T7b, T7a);
          }
        }
        {
          E T6D, T6S, T71, T74;
          T6D = FMA(KP773010453, T6C, T6v);
          T6S = ADD(T6K, T6R);
          Cr[WSCR(55)] = FNMS(KP903989293, T6S, T6D);
          Cr[WSCR(9)] = FMA(KP903989293, T6S, T6D);
          T71 = FNMS(KP773010453, T6Y, T6V);
          T74 = SUB(T72, T73);
          Ci[WSCI(9)] = NEG(FNMS(KP903989293, T74, T71));
          Ci[WSCI(55)] = FMA(KP903989293, T74, T71);
        }
        {
          E T6Z, T70, T75, T76;
          T6Z = FMA(KP773010453, T6Y, T6V);
          T70 = SUB(T6R, T6K);
          Ci[WSCI(23)] = FMA(KP903989293, T70, T6Z);
          Ci[WSCI(41)] = NEG(FNMS(KP903989293, T70, T6Z));
          T75 = FNMS(KP773010453, T6C, T6v);
          T76 = ADD(T73, T72);
          Cr[WSCR(41)] = FNMS(KP903989293, T76, T75);
          Cr[WSCR(23)] = FMA(KP903989293, T76, T75);
        }
        {
          E T79, T7g, T7l, T7o;
          T79 = FMA(KP773010453, T78, T77);
          T7g = ADD(T7c, T7f);
          Cr[WSCR(57)] = FNMS(KP941544065, T7g, T79);
          Cr[WSCR(7)] = FMA(KP941544065, T7g, T79);
          T7l = FMA(KP773010453, T7i, T7h);
          T7o = SUB(T7m, T7n);
          Ci[WSCI(7)] = FMA(KP941544065, T7o, T7l);
          Ci[WSCI(57)] = NEG(FNMS(KP941544065, T7o, T7l));
        }
        {
          E T7j, T7k, T7p, T7q;
          T7j = FNMS(KP773010453, T7i, T7h);
          T7k = SUB(T7f, T7c);
          Ci[WSCI(25)] = NEG(FNMS(KP941544065, T7k, T7j));
          Ci[WSCI(39)] = FMA(KP941544065, T7k, T7j);
          T7p = FNMS(KP773010453, T78, T77);
          T7q = ADD(T7m, T7n);
          Cr[WSCR(39)] = FMA(KP941544065, T7q, T7p);
          Cr[WSCR(25)] = FNMS(KP941544065, T7q, T7p);
        }
      }
      {
        E T99, T9L, T9g, T9W, T9C, T9M, T9z, T9V, T9v, Ta0, T9H, T9T,
         T9o, Ta1, T9G;
        E T9Q;
        {
          E T97, T98, T9x, T9y;
          T97 = FNMS(KP923879532, T7s, T7r);
          T98 = SUB(T8z, T8y);
          T99 = FNMS(KP831469612, T98, T97);
          T9L = FMA(KP831469612, T98, T97);
          {
            E T9c, T9f, T9A, T9B;
            T9c = FMA(KP534511135, T9b, T9a);
            T9f = FNMS(KP534511135, T9e, T9d);
            T9g = SUB(T9c, T9f);
            T9W = ADD(T9c, T9f);
            T9A = FMA(KP534511135, T9d, T9e);
            T9B = FNMS(KP534511135, T9a, T9b);
            T9C = SUB(T9A, T9B);
            T9M = ADD(T9B, T9A);
          }
          T9x = FNMS(KP923879532, T8w, T8v);
          T9y = SUB(T7z, T7w);
          T9z = FMA(KP831469612, T9y, T9x);
          T9V = FNMS(KP831469612, T9y, T9x);
          {
            E T9r, T9R, T9u, T9S, T9q, T9t;
            T9q = SUB(T8p, T8q);
            T9r = FNMS(KP831469612, T9q, T9p);
            T9R = FMA(KP831469612, T9q, T9p);
            T9t = SUB(T8j, T8g);
            T9u = FNMS(KP831469612, T9t, T9s);
            T9S = FMA(KP831469612, T9t, T9s);
            T9v = FMA(KP599376933, T9u, T9r);
            Ta0 = FMA(KP250486960, T9R, T9S);
            T9H = FNMS(KP599376933, T9r, T9u);
            T9T = FNMS(KP250486960, T9S, T9R);
          }
          {
            E T9k, T9O, T9n, T9P, T9j, T9m;
            T9j = SUB(T87, T86);
            T9k = FNMS(KP831469612, T9j, T9i);
            T9O = FMA(KP831469612, T9j, T9i);
            T9m = SUB(T7X, T80);
            T9n = FNMS(KP831469612, T9m, T9l);
            T9P = FMA(KP831469612, T9m, T9l);
            T9o = FMA(KP599376933, T9n, T9k);
            Ta1 = FMA(KP250486960, T9O, T9P);
            T9G = FNMS(KP599376933, T9k, T9n);
            T9Q = FNMS(KP250486960, T9P, T9O);
          }
        }
        {
          E T9h, T9w, T9F, T9I;
          T9h = FMA(KP881921264, T9g, T99);
          T9w = ADD(T9o, T9v);
          Cr[WSCR(53)] = FNMS(KP857728610, T9w, T9h);
          Cr[WSCR(11)] = FMA(KP857728610, T9w, T9h);
          T9F = FMA(KP881921264, T9C, T9z);
          T9I = SUB(T9G, T9H);
          Ci[WSCI(11)] = FMA(KP857728610, T9I, T9F);
          Ci[WSCI(53)] = NEG(FNMS(KP857728610, T9I, T9F));
        }
        {
          E T9D, T9E, T9J, T9K;
          T9D = FNMS(KP881921264, T9C, T9z);
          T9E = SUB(T9v, T9o);
          Ci[WSCI(21)] = NEG(FNMS(KP857728610, T9E, T9D));
          Ci[WSCI(43)] = FMA(KP857728610, T9E, T9D);
          T9J = FNMS(KP881921264, T9g, T99);
          T9K = ADD(T9G, T9H);
          Cr[WSCR(43)] = FMA(KP857728610, T9K, T9J);
          Cr[WSCR(21)] = FNMS(KP857728610, T9K, T9J);
        }
        {
          E T9N, T9U, T9Z, Ta2;
          T9N = FMA(KP881921264, T9M, T9L);
          T9U = ADD(T9Q, T9T);
          Cr[WSCR(59)] = FNMS(KP970031253, T9U, T9N);
          Cr[WSCR(5)] = FMA(KP970031253, T9U, T9N);
          T9Z = FMA(KP881921264, T9W, T9V);
          Ta2 = SUB(Ta0, Ta1);
          Ci[WSCI(5)] = NEG(FNMS(KP970031253, Ta2, T9Z));
          Ci[WSCI(59)] = FMA(KP970031253, Ta2, T9Z);
        }
        {
          E T9X, T9Y, Ta3, Ta4;
          T9X = FNMS(KP881921264, T9W, T9V);
          T9Y = SUB(T9T, T9Q);
          Ci[WSCI(27)] = FMA(KP970031253, T9Y, T9X);
          Ci[WSCI(37)] = NEG(FNMS(KP970031253, T9Y, T9X));
          Ta3 = FNMS(KP881921264, T9M, T9L);
          Ta4 = ADD(Ta1, Ta0);
          Cr[WSCR(37)] = FNMS(KP970031253, Ta4, Ta3);
          Cr[WSCR(27)] = FMA(KP970031253, Ta4, Ta3);
        }
      }
      {
        E T2z, T69, T3g, T6k, T60, T6a, T5X, T6j, T5L, T6p, T64, T6h,
         T4w, T6o, T65;
        E T6e;
        {
          E T2f, T2y, T5T, T5W;
          T2f = FMA(KP923879532, T2e, T27);
          T2y = ADD(T2o, T2x);
          T2z = FMA(KP980785280, T2y, T2f);
          T69 = FNMS(KP980785280, T2y, T2f);
          {
            E T2U, T3f, T5Y, T5Z;
            T2U = FNMS(KP098491403, T2T, T2M);
            T3f = FMA(KP098491403, T3e, T37);
            T3g = ADD(T2U, T3f);
            T6k = SUB(T3f, T2U);
            T5Y = FMA(KP098491403, T2M, T2T);
            T5Z = FNMS(KP098491403, T37, T3e);
            T60 = ADD(T5Y, T5Z);
            T6a = SUB(T5Y, T5Z);
          }
          T5T = FMA(KP923879532, T5S, T5P);
          T5W = ADD(T5U, T5V);
          T5X = FMA(KP980785280, T5W, T5T);
          T6j = FNMS(KP980785280, T5W, T5T);
          {
            E T5x, T6f, T5K, T6g, T5w, T5J;
            T5w = ADD(T5e, T5v);
            T5x = FMA(KP980785280, T5w, T4X);
            T6f = FNMS(KP980785280, T5w, T4X);
            T5J = ADD(T5H, T5I);
            T5K = FMA(KP980785280, T5J, T5G);
            T6g = FNMS(KP980785280, T5J, T5G);
            T5L = FNMS(KP049126849, T5K, T5x);
            T6p = FNMS(KP906347169, T6f, T6g);
            T64 = FMA(KP049126849, T5x, T5K);
            T6h = FMA(KP906347169, T6g, T6f);
          }
          {
            E T4i, T6c, T4v, T6d, T4h, T4u;
            T4h = ADD(T3Z, T4g);
            T4i = FMA(KP980785280, T4h, T3I);
            T6c = FNMS(KP980785280, T4h, T3I);
            T4u = ADD(T4s, T4t);
            T4v = FMA(KP980785280, T4u, T4r);
            T6d = FNMS(KP980785280, T4u, T4r);
            T4w = FNMS(KP049126849, T4v, T4i);
            T6o = FNMS(KP906347169, T6c, T6d);
            T65 = FMA(KP049126849, T4i, T4v);
            T6e = FMA(KP906347169, T6d, T6c);
          }
        }
        {
          E T3h, T5M, T63, T66;
          T3h = FMA(KP995184726, T3g, T2z);
          T5M = ADD(T4w, T5L);
          Cr[WSCR(63)] = FNMS(KP998795456, T5M, T3h);
          Cr[WSCR(1)] = FMA(KP998795456, T5M, T3h);
          T63 = FMA(KP995184726, T60, T5X);
          T66 = SUB(T64, T65);
          Ci[WSCI(1)] = NEG(FNMS(KP998795456, T66, T63));
          Ci[WSCI(63)] = FMA(KP998795456, T66, T63);
        }
        {
          E T61, T62, T67, T68;
          T61 = FNMS(KP995184726, T60, T5X);
          T62 = SUB(T5L, T4w);
          Ci[WSCI(31)] = FMA(KP998795456, T62, T61);
          Ci[WSCI(33)] = NEG(FNMS(KP998795456, T62, T61));
          T67 = FNMS(KP995184726, T3g, T2z);
          T68 = ADD(T65, T64);
          Cr[WSCR(33)] = FNMS(KP998795456, T68, T67);
          Cr[WSCR(31)] = FMA(KP998795456, T68, T67);
        }
        {
          E T6b, T6i, T6n, T6q;
          T6b = FMA(KP995184726, T6a, T69);
          T6i = ADD(T6e, T6h);
          Cr[WSCR(49)] = FNMS(KP740951125, T6i, T6b);
          Cr[WSCR(15)] = FMA(KP740951125, T6i, T6b);
          T6n = FMA(KP995184726, T6k, T6j);
          T6q = SUB(T6o, T6p);
          Ci[WSCI(15)] = FMA(KP740951125, T6q, T6n);
          Ci[WSCI(49)] = NEG(FNMS(KP740951125, T6q, T6n));
        }
        {
          E T6l, T6m, T6r, T6s;
          T6l = FNMS(KP995184726, T6k, T6j);
          T6m = SUB(T6h, T6e);
          Ci[WSCI(17)] = NEG(FNMS(KP740951125, T6m, T6l));
          Ci[WSCI(47)] = FMA(KP740951125, T6m, T6l);
          T6r = FNMS(KP995184726, T6a, T69);
          T6s = ADD(T6o, T6p);
          Cr[WSCR(47)] = FMA(KP740951125, T6s, T6r);
          Cr[WSCR(17)] = FNMS(KP740951125, T6s, T6r);
        }
      }
      {
        E T7B, T8N, T7Q, T8Y, T8E, T8O, T8B, T8X, T8t, T92, T8J, T8V,
         T8a, T93, T8I;
        E T8S;
        {
          E T7t, T7A, T8x, T8A;
          T7t = FMA(KP923879532, T7s, T7r);
          T7A = ADD(T7w, T7z);
          T7B = FMA(KP831469612, T7A, T7t);
          T8N = FNMS(KP831469612, T7A, T7t);
          {
            E T7I, T7P, T8C, T8D;
            T7I = FMA(KP303346683, T7H, T7E);
            T7P = FNMS(KP303346683, T7O, T7L);
            T7Q = ADD(T7I, T7P);
            T8Y = SUB(T7P, T7I);
            T8C = FNMS(KP303346683, T7E, T7H);
            T8D = FMA(KP303346683, T7L, T7O);
            T8E = ADD(T8C, T8D);
            T8O = SUB(T8D, T8C);
          }
          T8x = FMA(KP923879532, T8w, T8v);
          T8A = ADD(T8y, T8z);
          T8B = FMA(KP831469612, T8A, T8x);
          T8X = FNMS(KP831469612, T8A, T8x);
          {
            E T8l, T8T, T8s, T8U, T8k, T8r;
            T8k = ADD(T8g, T8j);
            T8l = FMA(KP831469612, T8k, T8d);
            T8T = FNMS(KP831469612, T8k, T8d);
            T8r = ADD(T8p, T8q);
            T8s = FMA(KP831469612, T8r, T8o);
            T8U = FNMS(KP831469612, T8r, T8o);
            T8t = FMA(KP148335987, T8s, T8l);
            T92 = FMA(KP741650546, T8T, T8U);
            T8J = FNMS(KP148335987, T8l, T8s);
            T8V = FNMS(KP741650546, T8U, T8T);
          }
          {
            E T82, T8Q, T89, T8R, T81, T88;
            T81 = ADD(T7X, T80);
            T82 = FMA(KP831469612, T81, T7U);
            T8Q = FNMS(KP831469612, T81, T7U);
            T88 = ADD(T86, T87);
            T89 = FMA(KP831469612, T88, T85);
            T8R = FNMS(KP831469612, T88, T85);
            T8a = FMA(KP148335987, T89, T82);
            T93 = FMA(KP741650546, T8Q, T8R);
            T8I = FNMS(KP148335987, T82, T89);
            T8S = FNMS(KP741650546, T8R, T8Q);
          }
        }
        {
          E T7R, T8u, T8H, T8K;
          T7R = FMA(KP956940335, T7Q, T7B);
          T8u = ADD(T8a, T8t);
          Cr[WSCR(61)] = FNMS(KP989176509, T8u, T7R);
          Cr[WSCR(3)] = FMA(KP989176509, T8u, T7R);
          T8H = FMA(KP956940335, T8E, T8B);
          T8K = SUB(T8I, T8J);
          Ci[WSCI(3)] = FMA(KP989176509, T8K, T8H);
          Ci[WSCI(61)] = NEG(FNMS(KP989176509, T8K, T8H));
        }
        {
          E T8F, T8G, T8L, T8M;
          T8F = FNMS(KP956940335, T8E, T8B);
          T8G = SUB(T8t, T8a);
          Ci[WSCI(29)] = NEG(FNMS(KP989176509, T8G, T8F));
          Ci[WSCI(35)] = FMA(KP989176509, T8G, T8F);
          T8L = FNMS(KP956940335, T7Q, T7B);
          T8M = ADD(T8I, T8J);
          Cr[WSCR(35)] = FMA(KP989176509, T8M, T8L);
          Cr[WSCR(29)] = FNMS(KP989176509, T8M, T8L);
        }
        {
          E T8P, T8W, T91, T94;
          T8P = FMA(KP956940335, T8O, T8N);
          T8W = ADD(T8S, T8V);
          Cr[WSCR(51)] = FNMS(KP803207531, T8W, T8P);
          Cr[WSCR(13)] = FMA(KP803207531, T8W, T8P);
          T91 = FNMS(KP956940335, T8Y, T8X);
          T94 = SUB(T92, T93);
          Ci[WSCI(13)] = NEG(FNMS(KP803207531, T94, T91));
          Ci[WSCI(51)] = FMA(KP803207531, T94, T91);
        }
        {
          E T8Z, T90, T95, T96;
          T8Z = FMA(KP956940335, T8Y, T8X);
          T90 = SUB(T8V, T8S);
          Ci[WSCI(19)] = FMA(KP803207531, T90, T8Z);
          Ci[WSCI(45)] = NEG(FNMS(KP803207531, T90, T8Z));
          T95 = FNMS(KP956940335, T8O, T8N);
          T96 = ADD(T93, T92);
          Cr[WSCR(45)] = FNMS(KP803207531, T96, T95);
          Cr[WSCR(19)] = FMA(KP803207531, T96, T95);
        }
      }
    }
  }
}
